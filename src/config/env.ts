...
export interface AppConfig {
  ...
  recruiterOutreach: { enabled: boolean; dryRun: boolean; activation: "disabled" | "canary" | "live"; liveActivationConfirmed: boolean; discoveryProvider: "public-web"; minConfidence: number; requireVerifiedEmail: boolean; maxContactsPerApplication: number; maxMessagesPerDay: number; maxMessagesPerHour: number; followUpEnabled: boolean; followUpDayOffsets: number[]; genericEmailFallback: boolean; };
...
export function loadConfig(): AppConfig {
  const emailEnabled = booleanValue("EMAIL_ENABLED", process.env.EMAIL_ENABLED, false); const resumeTailoringEnabled = booleanValue("RESUME_TAILORING_ENABLED", process.env.RESUME_TAILORING_ENABLED, false); const gmailEnabled = booleanValue("GMAIL_ENABLED", process.env.GMAIL_ENABLED, false); const gmailAccountTier = process.env.GMAIL_ACCOUNT_TIER ?? "consumer";
  if (gmailAccountTier !== "consumer" && gmailAccountTier !== "workspace") throw new Error("GMAIL_ACCOUNT_TIER must be consumer or workspace");
  const gmailDailySendLimit = gmailAccountTier === "workspace" ? 2000 : 500; const recruiterOutreachEnabled = booleanValue("RECRUITER_OUTREACH_ENABLED", process.env.RECRUITER_OUTREACH_ENABLED, false); const recruiterDryRun = booleanValue("RECRUITER_OUTREACH_DRY_RUN", process.env.RECRUITER_OUTREACH_DRY_RUN, true); const recruiterActivation = process.env.RECRUITER_OUTREACH_ACTIVATION ?? "disabled";
  ...
  if (recruiterProvider !== "public-web") throw new Error("RECRUITER_DISCOVERY_PROVIDER must be public-web");
  const recruiterMaxMessagesPerDay = positiveInteger("RECRUITER_MAX_MESSAGES_PER_DAY", process.env.RECRUITER_MAX_MESSAGES_PER_DAY ?? "50"); const recruiterMaxMessagesPerHour = positiveInteger("RECRUITER_MAX_MESSAGES_PER_HOUR", process.env.RECRUITER_MAX_MESSAGES_PER_HOUR ?? "2");
  const configuredJobSources = process.env.JOB_SOURCES?.trim();
  return {
    nodeEnv: process.env.NODE_ENV ?? "development", logLevel: process.env.LOG_LEVEL ?? "info", automationEnabled: booleanValue("AUTOMATION_ENABLED", process.env.AUTOMATION_ENABLED, true), applicationDryRun: booleanValue("APPLICATION_DRY_RUN", process.env.APPLICATION_DRY_RUN, true), outboundEnabled: booleanValue("OUTBOUND_ENABLED", process.env.OUTBOUND_ENABLED, false), discoveryEnabled: booleanValue("JOB_DISCOVERY_ENABLED", process.env.JOB_DISCOVERY_ENABLED, true), discoveryIntervalMs: positiveInteger("JOB_DISCOVERY_INTERVAL_MS", process.env.JOB_DISCOVERY_INTERVAL_MS ?? "900000"), applicationQueueIntervalMs: positiveInteger("APPLICATION_QUEUE_INTERVAL_MS", process.env.APPLICATION_QUEUE_INTERVAL_MS ?? "30000"), staleSubmissionCheckIntervalMs: positiveInteger("STALE_SUBMISSION_CHECK_INTERVAL_MS", process.env.STALE_SUBMISSION_CHECK_INTERVAL_MS ?? "300000"), staleSubmissionThresholdMinutes: positiveInteger("STALE_SUBMISSION_THRESHOLD_MINUTES", process.env.STALE_SUBMISSION_THRESHOLD_MINUTES ?? "30"), followUpIntervalMs: positiveInteger("FOLLOW_UP_INTERVAL_MS", process.env.FOLLOW_UP_INTERVAL_MS ?? "300000"), interviewReminderIntervalMs: positiveInteger("INTERVIEW_REMINDER_INTERVAL_MS", process.env.INTERVIEW_REMINDER_INTERVAL_MS ?? "300000"), jobSources: configuredJobSources || defaultJobSources(), genericApplicationAdapterEnabled: booleanValue("GENERIC_APPLICATION_ADAPTER_ENABLED", process.env.GENERIC_APPLICATION_ADAPTER_ENABLED, true), applicationRateLimitPerDay: positiveInteger("APPLICATION_RATE_LIMIT_PER_DAY", process.env.APPLICATION_RATE_LIMIT_PER_DAY ?? "50"), applicationCompanyRateLimitPerDay: positiveInteger("APPLICATION_COMPANY_RATE_LIMIT_PER_DAY", process.env.APPLICATION_COMPANY_RATE_LIMIT_PER_DAY ?? "5"),
    recruiterOutreach: { enabled: recruiterOutreachEnabled, dryRun: recruiterDryRun, activation: recruiterActivation as "disabled" | "canary" | "live", liveActivationConfirmed: recruiterLiveActivationConfirmed, discoveryProvider: recruiterProvider as "public-web", minConfidence: boundedInteger("RECRUITER_MIN_CONFIDENCE", process.env.RECRUITER_MIN_CONFIDENCE ?? "80", 0, 100), requireVerifiedEmail: booleanValue("RECRUITER_REQUIRE_VERIFIED_EMAIL", process.env.RECRUITER_REQUIRE_VERIFIED_EMAIL, true), maxContactsPerApplication: positiveInteger("RECRUITER_MAX_CONTACTS_PER_APPLICATION", process.env.RECRUITER_MAX_CONTACTS_PER_APPLICATION ?? "3"), maxMessagesPerDay: recruiterMaxMessagesPerDay, maxMessagesPerHour: recruiterMaxMessagesPerHour, followUpEnabled: booleanValue("RECRUITER_FOLLOWUP_ENABLED", process.env.RECRUITER_FOLLOWUP_ENABLED, true), followUpDayOffsets: dayOffsets("RECRUITER_FOLLOWUP_DAY_OFFSETS", process.env.RECRUITER_FOLLOWUP_DAY_OFFSETS ?? "4,10,18"), genericEmailFallback: booleanValue("RECRUITER_GENERIC_EMAIL_FALLBACK", process.env.RECRUITER_GENERIC_EMAIL_FALLBACK, true) },
