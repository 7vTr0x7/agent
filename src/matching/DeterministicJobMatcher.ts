...
const ALIASES: Record<string, string[]> = {
  react: ["react", "reactjs", "react.js"], "react.js": ["react", "reactjs", "react.js"], nextjs: ["next", "nextjs", "next.js", "next js"], "next.js": ["next", "nextjs", "next.js", "next js"],
  node: ["node", "nodejs", "node.js", "node js"], "node.js": ["node", "nodejs", "node.js", "node js"], express: ["express", "expressjs", "express.js", "express js"], "express.js": ["express", "expressjs", "express.js", "express js"],
  javascript: ["javascript", "js", "ecmascript"], js: ["javascript", "js", "ecmascript"], typescript: ["typescript", "ts"], ts: ["typescript", "ts"], "redux toolkit": ["redux toolkit", "@reduxjs/toolkit"], redux: ["redux", "redux toolkit", "@reduxjs/toolkit"],
  "tailwind css": ["tailwind css", "tailwind"], tailwind: ["tailwind css", "tailwind"], mongodb: ["mongodb", "mongo db", "mongo"], "react testing library": ["react testing library", "rtl"], rtl: ["react testing library", "rtl"], postgresql: ["postgresql", "postgres", "postgre sql"], postgres: ["postgresql", "postgres", "postgre sql"]
};
...
export class DeterministicJobMatcher {
  ...
  evaluate(job: JobOpportunity, profile: CandidateProfile): DeterministicMatchResult {
    const title = normalize(job.title); const text = normalize(`${job.title}\n${job.description}`); const evidence: MatchEvidence[] = [];
    const matchedSkills = profile.skills.filter((skill) => containsSkill(text, skill)); const missingSkills = profile.skills.filter((skill) => !containsSkill(text, skill));
    matchedSkills.forEach((skill) => evidence.push({ type: "SKILL_MATCH", detail: `Candidate skill appears in the posting: ${skill}` })); missingSkills.forEach((skill) => evidence.push({ type: "SKILL_GAP", detail: `Candidate skill is not mentioned in the posting: ${skill}` }));
    const titleMatch = profile.targetTitles.some((target) => containsPhrase(title, normalize(target))); if (titleMatch) evidence.push({ type: "TITLE_MATCH", detail: "Job title matches a candidate target title." });
    const geography = classifyGeography(job); const freshness = classifyFreshness(job.postedAt, this.now()); const seniority = classifySeniority(title, text, profile.yearsExperience); const technicalOrientation = classifyTechnicalOrientation(title, text);
    ...
    const requiredYears = extractRequiredYears(text); const experienceRange = extractExperienceRange(text);
    ...
    if (seniority !== "UNKNOWN") evidence.push({ type: "SENIORITY", detail: `Detected seniority: ${seniority}.` }); evidence.push({ type: "LOCATION", detail: `Detected geography: ${geography}.` }, { type: "FRESHNESS", detail: `Detected freshness: ${freshness}.` });
    const salary = salaryAdjustment(job.description, profile); if (salary !== null) evidence.push({ type: "SALARY", detail: salary < 0 ? "Explicit salary appears materially below configured compensation." : "Explicit salary is not materially adverse." });
    ...
    if (technicalOrientation === "BACKEND_FOCUSED") score -= 25; if (geography === "REMOTE_WORLDWIDE") score -= 8; if (geography === "UNKNOWN") score -= 12; if (freshness === "STALE") score -= 25; if (freshness === "MODERATELY_OLD") score -= 10; if (seniority === "SENIOR_HIGH" || seniority === "LEAD" || seniority === "STAFF") score -= 18; if (seniority === "SENIOR_COMPATIBLE") score -= 4;
    ...
    const riskReview = (salary !== null && salary < 0) || geography === "REMOTE_WORLDWIDE" || seniority === "SENIOR_HIGH" || seniority === "LEAD" || seniority === "STAFF" || technicalOrientation === "BACKEND_FOCUSED" || experienceMismatch(requiredYears, experienceRange, profile.yearsExperience);
...
function classifyTechnicalOrientation(title: string, text: string): TechnicalOrientation {
  ...
  if (!web) return "UNRELATED"; if (FULL_STACK_TITLE.test(title)) return "FULL_STACK"; if (FRONTEND_TITLE.test(title)) return "REACT_WEB"; return "FRONTEND";
}
...
function extractRequiredYears(text: string): number | null { const matches = [...text.matchAll(/(?:minimum|at least|required|must have)\s+(\d+(?:\.\d+)?)\s*\+?\s*years?(?:\s+of)?\s+experience/g), ...text.matchAll(/(?:experience|exp)\s*[:\-]?\s*(\d+(?:\.\d+)?)\s*\+\s*years?/g), ...text.matchAll(/\b(\d+(?:\.\d+)?)\s*\+\s*years?\s+(?:of\s+)?experience\s+(?:is\s+)?(?:required|mandatory|minimum)\b/g), ...text.matchAll(/\b(\d+(?:\.\d+)?)\s*\+\s*years?\s+required\b/g)]; const values = matches.map((match) => Number(match[1])).filter(Number.isFinite); return values.length ? Math.max(...values) : null; }
...
function experienceMismatch(required: number | null, range: { min: number; max: number } | null, years: number): boolean { return (required !== null && required > years && required < 5) || (range !== null && range.min > years); }
