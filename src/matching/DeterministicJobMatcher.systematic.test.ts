...
describe("matcher systematic calibration regressions", () => {
  ...
  it("keeps strong worldwide remote target roles eligible", () => { const result = run("React and TypeScript. Worldwide remote.", "Frontend Engineer", { location: "Worldwide", country: null, workplaceType: "remote" }); expect(result.geography).toBe("REMOTE_WORLDWIDE"); expect(result.decision).toBe("APPLY"); });
