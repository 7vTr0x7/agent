...
function response(status: number, body: unknown, headers: Record<string, string> = {}): Response { return new Response(JSON.stringify(body), { status, headers }); }

describe("GmailApiMailbox", () => {
  const oauth = { getAccessToken: jest.fn().mockResolvedValue("access-token"), invalidateAccessToken: jest.fn() } as unknown as GmailOAuthClient;
  beforeEach(() => jest.clearAllMocks());

  it("retries transient listMessages failures", async () => {
    const fetchImpl = jest.fn().mockResolvedValueOnce(response(503, {})).mockResolvedValueOnce(response(200, { messages: [{ id: "message-1" }] }));
    ...
    expect(fetchImpl).toHaveBeenCalledTimes(2); expect(sleepImpl).toHaveBeenCalledWith(10);
  });

  it("honors Retry-After for rate-limited getMessage calls", async () => {
    const fetchImpl = jest.fn().mockResolvedValueOnce(response(429, {}, { "Retry-After": "2" })).mockResolvedValueOnce(response(200, { id: "message-1", threadId: "thread-1", payload: { headers: [{ name: "From", value: "Recruiter <recruiter@example.com>" }, { name: "Subject", value: "Frontend Engineer" }], mimeType: "text/plain", body: { data: Buffer.from("Hello").toString("base64url") } } }));
    const sleepImpl = jest.fn().mockResolvedValue(undefined);
    const mailbox = new GmailApiMailbox({ oauth, userEmail: "candidate@example.com", fetchImpl, retryDelayMs: 10, sleepImpl });
    ...
    expect(fetchImpl).toHaveBeenCalledTimes(2); expect(sleepImpl).toHaveBeenCalledWith(2000);
  });

  it("refreshes the access token once after a 401", async () => {
    const fetchImpl = jest.fn().mockResolvedValueOnce(response(401, {})).mockResolvedValueOnce(response(200, { messages: [{ id: "message-1" }] }));
    const mailbox = new GmailApiMailbox({ oauth, userEmail: "candidate@example.com", fetchImpl });
    await expect(mailbox.listMessages("newer_than:1d", 10)).resolves.toEqual(["message-1"]);
    expect(oauth.invalidateAccessToken).toHaveBeenCalledTimes(1); expect(fetchImpl).toHaveBeenCalledTimes(2);
  });
  ...
  it("does not retry sendMessage after a transient failure", async () => {
    const fetchImpl = jest.fn().mockResolvedValue(response(503, {})); const sleepImpl = jest.fn().mockResolvedValue(undefined);
    const mailbox = new GmailApiMailbox({ oauth, userEmail: "candidate@example.com", fetchImpl, retryDelayMs: 10, sleepImpl });
    ...
    expect(fetchImpl).toHaveBeenCalledTimes(1); expect(sleepImpl).not.toHaveBeenCalled();
  });

  it("does not retry permanent GET failures after the single token-refresh attempt", async () => {
    const fetchImpl = jest.fn().mockResolvedValue(response(401, {})); const sleepImpl = jest.fn().mockResolvedValue(undefined);
    ...
    expect(fetchImpl).toHaveBeenCalledTimes(2); expect(oauth.invalidateAccessToken).toHaveBeenCalledTimes(1); expect(sleepImpl).not.toHaveBeenCalled();
  });

  it("treats a POST timeout as a single ambiguous attempt and never retries it", async () => {
    const fetchImpl = jest.fn().mockImplementation((_url: string, init: RequestInit) => new Promise((_resolve, reject) => {
      const signal = init.signal as AbortSignal;
      signal.addEventListener("abort", () => reject(new DOMException("aborted", "AbortError")), { once: true });
    }));
    const sleepImpl = jest.fn().mockResolvedValue(undefined);
    const mailbox = new GmailApiMailbox({ oauth, userEmail: "candidate@example.com", fetchImpl, requestTimeoutMs: 1000, retryDelayMs: 10, sleepImpl });
    await expect(mailbox.sendMessage({ to: "recruiter@example.com", subject: "Frontend Engineer", bodyText: "Hello" })).rejects.toThrow("Gmail API request timed out after 1000ms.");
    expect(fetchImpl).toHaveBeenCalledTimes(1); expect(sleepImpl).not.toHaveBeenCalled();
  });
