...
export class SourceRunTracker {
      ...
      await client.query(
        `UPDATE source_runs
         SET status = $2, finished_at = NOW(), fetched_count = $3,
             inserted_count = $4, duplicate_count = $5, error_summary = $6
         WHERE id = $1`,
        ...
        await client.query(
          `UPDATE sources
           SET consecutive_failures = 0, last_success_at = NOW(),
               disabled_until = NULL, updated_at = NOW()
           WHERE id = $1`,
          [sourceId]
        );
      }
  ...
  async recordError(runId: string, sourceId: string, error: SourceErrorInput): Promise<void> {
    await this.database.transaction(async (client) => {
      await client.query(
      ...
      await client.query(`UPDATE source_runs SET error_count = error_count + 1 WHERE id = $1`, [runId]);
      await client.query(
        `UPDATE sources SET consecutive_failures = consecutive_failures + 1, updated_at = NOW() WHERE id = $1`,
        [sourceId]
      );
    });
  }
  ...
  async markReviewRequired(sourceId: string, threshold: number): Promise<void> {
    await this.database.query(
      `UPDATE sources
       SET status = CASE WHEN consecutive_failures >= $2 THEN 'REVIEW_REQUIRED' ELSE status END,
           updated_at = NOW()
       WHERE id = $1`,
      [sourceId, threshold]
    );
  ...
  private async upsertSource(client: PoolClient, descriptor: SourceDescriptor): Promise<void> {
    await client.query(
      `INSERT INTO sources (id, name, source_type, status)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT (id) DO UPDATE SET
         name = EXCLUDED.name, source_type = EXCLUDED.source_type, updated_at = NOW()`,
