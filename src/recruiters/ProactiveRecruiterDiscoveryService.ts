...
function countSearchResults(raw:string,normalized:string,urls:string[]){try{const parsed=JSON.parse(raw) as{results?:unknown[];web?:{results?:unknown[]};organic?:unknown[]};const count=parsed.results?.length??parsed.web?.results?.length??parsed.organic?.length;if(typeof count==="number")return count;}catch{}const markers=normalized.match(/(?:\n|^)(?:\d+[.)]|result|sponsored|organic result|web result)\b/gi)?.length??0;return Math.max(markers,urls.length>0?urls.length:0);}
function isPublicProfileUrl(value:string){try{const url=new URL(value),host=url.hostname.toLowerCase();if(SEARCH_HOSTS.has(host))return false;if(host.endsWith("linkedin.com")&&/^\/in\//i.test(url.pathname))return true;return PROFILE_PATH.test(url.pathname);}catch{return false;}}
function identityKey(c:ProactiveRecruiterDiscoveryCandidate){if(c.email)return`email:${c.email.toLowerCase()}`;if(c.discoveryUrl.startsWith("https://www.linkedin.com/in/"))return`linkedin:${c.discoveryUrl}`;if(isPublicProfileUrl(c.discoveryUrl))return`profile:${canonicalUrl(c.discoveryUrl)}`;return`person:${c.recruiterName.toLowerCase().replace(/[^a-z0-9]+/g," ").trim()}|${c.employerDomain??c.employer.toLowerCase().replace(/[^a-z0-9]+/g,"")}`;}
...
function hasHiringEvidence(e:string){return CURRENT_HIRING_EVIDENCE.test(e)||(RECENT_HIRING_EVIDENCE.test(e)&&/hiring|recruiting|recruiter|role|position|opening/i.test(e));}
function classifyEvidenceFreshness(e:string,now:Date):ProactiveRecruiterDiscoveryCandidate["evidenceFreshness"]{const l=e.toLowerCase();if(CURRENT_HIRING_EVIDENCE.test(l))return"current";if(RECENT_HIRING_EVIDENCE.test(l))return"recent";const years=[...l.matchAll(/\b(20\d{2})\b/g)].map(m=>Number(m[1])).filter(Number.isFinite);if(years.some(y=>now.getFullYear()-y>=2))return"historical";if(/historical|previously|formerly|past hiring|used to recruit/.test(l))return"historical";return"unknown";}
function inferEvidenceDate(e:string,now:Date){const years=[...e.matchAll(/\b(20\d{2})\b/g)].map(m=>Number(m[1])).filter(Number.isFinite),y=years.find(v=>v<=now.getFullYear());return y?new Date(Date.UTC(y,0,1)):now;}
