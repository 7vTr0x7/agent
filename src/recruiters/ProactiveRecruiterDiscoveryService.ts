  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "user-agent": "Mozilla/5.0 (compatible; job-agent-proactive-recruiter/8.0)",
        ...(headers ?? {})
      }
    });
...
const statFor = (metrics: ProactiveRecruiterDiscoveryMetrics, id: SourceId): ProactiveRecruiterDiscoveryMetrics["sourceStats"][string] => metrics.sourceStats[id] ?? (metrics.sourceStats[id] = {
  attempted: 0, succeeded: 0, empty: 0, timeouts: 0, http403: 0, http429: 0, http5xx: 0, otherHttpErrors: 0, rawResults: 0, urls: 0, candidates: 0, duplicates: 0
});

const reject = (metrics: ProactiveRecruiterDiscoveryMetrics, reason: string): void => {
...
async function fetchSource(source: Source, fetcher: ProactiveRecruiterDiscoveryOptions["fetchText"], signal: AbortSignal | undefined, metrics: ProactiveRecruiterDiscoveryMetrics): Promise<string | null> {
  ...
  for (let attempt = 0; attempt <= RETRIES; attempt += 1) {
    ...
    const result = fetcher
      ? { text: await fetcher(source.url, signal, source.headers), status: 200 }
      : await DEFAULT_FETCH(source.url, signal, source.headers);
    if (result.text) {
      stats.succeeded += 1;
...
export class ProactiveRecruiterDiscoveryService {
  ...
  buildQueries(profile: CandidateProfileLike): string[] {
    ...
    return [...new Set(returnQueries)].slice(0, this.maxQueries);
  }
    ...
    for (const query of queries) {
      ...
      for (const { source, text: raw } of pages) {
        ...
        for (const url of candidateUrls) {
          ...
          const result = this.buildCandidate(profile, url, evidence, query, source.id, metrics);
          if (!result) continue;
        ...
        if (candidates.size < this.targetCandidates) {
          ...
          if (emailCandidate) {
            const key = identityKey(emailCandidate);
            if (candidates.has(key)) {
              metrics.duplicates += 1;
              stats.duplicates += 1;
              candidates.set(key, mergeCandidates(candidates.get(key) as ProactiveRecruiterDiscoveryCandidate, emailCandidate));
            } else {
              stats.candidates += 1;
              candidates.set(key, emailCandidate);
            }
          }
        }
      }
  ...
  private buildCandidate(profile: CandidateProfileLike, url: string, evidence: string, query: string, source: SourceId, metrics: ProactiveRecruiterDiscoveryMetrics): ProactiveRecruiterDiscoveryCandidate | null {
    ...
    if (hiringEvidence) metrics.hiringEvidenceAccepted += 1;
  ...
  private buildCandidateFromPage(profile: CandidateProfileLike, page: string, query: string, source: SourceId, metrics: ProactiveRecruiterDiscoveryMetrics): ProactiveRecruiterDiscoveryCandidate | null {
    ...
    const roleMatch = this.matcher.match(profile, page, `${page} ${query}`);
    if (!roleMatch.score) return null;
    ...
    if (employer.name === "Unknown employer") return null;
    metrics.recruiterRoleMatches += 1;
    metrics.recruiterEvidenceMatches += 1;
    metrics.identityValidated += 1;
...
function buildEvidence(text: string, url: string, query: string): string {
  const index = text.toLowerCase().indexOf(url.toLowerCase());
  const around = index >= 0 ? text.slice(Math.max(0, index - 1400), Math.min(text.length, index + 1800)) : text.slice(0, 2600);
...
function isLikelyPersonName(value: string): boolean {
  ...
  if (/^(Technical|Senior|Lead|Principal|Talent|Human|Resource|Hiring|Recruiter|Recruiting|Manager|India|Bangalore|Bengaluru|React|Frontend|Software|Engineering)\b/i.test(normalized)) return false;
...
function extractEmployer(evidence: string, email?: string): { name: string; domain?: string } {
  ...
  if (name) return emailDomain && !GENERIC_EMAIL_DOMAINS.has(emailDomain) ? { name, domain: emailDomain } : { name };
