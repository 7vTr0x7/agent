...
export class ProactiveRecruiterTaskHandler {
  ...
  async handleDiscovery(payload: ProactiveRecruiterDiscoveryPayload): Promise<void> {
    ...
    const profile: CandidateProfile = { id: payload.candidateProfileId, yearsExperience: payload.yearsExperience, skills: [...payload.skills], targetTitles: [...payload.targetRoles], location: payload.location, fullName: payload.candidateName, standardizedAnswers: { preferredLocations: preferredLocations.join(", ") } };
    const discovered = await this.discovery.discover({ targetRoles: [...profile.targetTitles], skills: [...profile.skills], yearsExperience: profile.yearsExperience, preferredLocations, remoteEligible: payload.remoteEligible });
    const ranked = rankProactiveRecruiters(discovered.map((candidate, index) => ({ id: candidate.discoveryUrl || `${candidate.recruiterName}:${index}`, roleMatchScore: candidate.roleMatchScore, hiringEvidenceScore: candidate.hiringEvidenceScore, evidenceFreshnessScore: freshnessScore(candidate.evidenceFreshness), employerRelevanceScore: employerRelevance(candidate.employer, preferredLocations, payload.remoteEligible ?? false), emailConfidenceScore: emailScore(candidate.emailStatus), locationRelevanceScore: locationScore(candidate.discoveryEvidence.join(" "), preferredLocations, payload.remoteEligible ?? false), suppressed: false })));
    ...
    const emailProvider = createRecruiterDiscoveryProvider({ provider: "public-web" });
    ...
    for (const rankedCandidate of ranked.slice(0, Math.max(1, Math.min(payload.maxCandidates, this.options.maxCandidatesPerRun)))) {
      ...
      persisted += 1;
      let emailContact: { email?: string; fullName?: string; linkedinProfileUrl?: string } | undefined = candidate.email ? { email: candidate.email, fullName: candidate.recruiterName, linkedinProfileUrl: candidate.discoveryUrl } : undefined;
      if (!emailContact?.email) {
        try {
          const emailResult = await emailProvider.discoverEmails?.({ companyName: candidate.employer, companyDomain: candidate.employerDomain, jobTitle: candidate.targetRoles[0] ?? candidate.recruiterRole, jobDescription: candidate.discoveryEvidence.join(" "), location: preferredLocations.join(", "), candidateProfileId: payload.candidateProfileId });
          const contacts = emailResult?.contacts ?? [];
          const normalizedName = normalizePersonName(candidate.recruiterName);
          emailContact = contacts.find((contact) => Boolean(contact.email) && ((contact.linkedinProfileUrl && contact.linkedinProfileUrl.toLowerCase() === candidate.discoveryUrl.toLowerCase()) || (contact.fullName && normalizePersonName(contact.fullName) === normalizedName)));
        } catch (error) {
          this.logger.error({ error: error instanceof Error ? error.message : String(error), recruiter: candidate.recruiterName }, "Proactive recruiter email discovery failed");
        }
      }
      if (!emailContact?.email) continue;
      emailCandidates += 1;
      try {
        const verification = await verifier(emailContact.email);
        const status = normalizeEmailStatus(verification.status, verification.verificationEvidence);
        await this.repository.enrichCandidateEmail({ recruiterContactId, email: emailContact.email, emailStatus: status, verificationEvidence: verification.verificationEvidence ?? [] });
        const mailboxEvidence = hasExplicitMailboxEvidence(verification.verificationEvidence ?? []);
        if (mailboxEvidence && status === "VERIFIED") verifiedEmails += 1;
        candidate.email = emailContact.email.toLowerCase(); candidate.emailStatus = status; candidate.verificationEvidence = verification.verificationEvidence ?? [];
      } catch (error) {
        this.logger.error({ error: error instanceof Error ? error.message : String(error), recruiter: candidate.recruiterName }, "Proactive recruiter email verification failed");
        candidate.emailStatus = "UNVERIFIED"; candidate.verificationEvidence = [];
      }
      const mailboxEvidence = hasExplicitMailboxEvidence(candidate.verificationEvidence ?? []);
      const canonicalEligible = isEligibleForRealRecruiterSend({ verified: mailboxEvidence, mailboxEvidence, verificationEvidence: candidate.verificationEvidence ?? [], emailStatus: mailboxEvidence ? "VERIFIED" : candidate.emailStatus, verificationStatus: mailboxEvidence ? "mailbox_verified" : candidate.emailStatus === "LIKELY" ? "domain_mx_verified" : "public-web-unverified", relevanceStatus: candidate.evidenceFreshness === "current" ? "CURRENT" : candidate.evidenceFreshness === "recent" ? "RECENT" : candidate.evidenceFreshness === "historical" ? "HISTORICAL" : "UNKNOWN", suppressed: false });
      if (!canonicalEligible || !candidate.email || candidate.emailStatus === "INVALID") continue;
      const campaign = await this.repository.createProactiveCampaign({ recruiterContactId, candidateProfileId: payload.candidateProfileId, targetRoles: [...profile.targetTitles], subject: `Frontend / React / Next.js opportunities — ${profile.fullName ?? "Candidate"}`, body: buildProactiveMessage(profile, candidate) });
...
function normalizePersonName(value: string): string { return value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim(); }
function buildProactiveMessage(profile: CandidateProfile, candidate: { recruiterName: string; recruiterRole: string; employer: string; targetRoles: string[]; evidenceFreshness: string; discoveryEvidence: string[] }): string { const name = profile.fullName?.trim() || [profile.firstName, profile.lastName].filter(Boolean).join(" ") || "Candidate"; const roles = profile.targetTitles.length ? profile.targetTitles.slice(0, 3).join(" / ") : "Frontend / React / Next.js"; const skills = profile.skills.slice(0, 5).join(", "); const location = profile.location ? ` I’m currently based in ${profile.location}.` : ""; const evidenceLine = candidate.evidenceFreshness === "current" ? "Your public recruiting information appears relevant to these kinds of roles." : candidate.evidenceFreshness === "recent" ? "Your recent public recruiting information appears relevant to these kinds of roles." : "Your public recruiting background appears relevant to these kinds of roles."; return [`Hi ${candidate.recruiterName.split(" ")[0] || "there"},`, "", `I’m ${name}, and I’m exploring ${roles} opportunities.${location}`, `I have ${profile.yearsExperience} years of experience with ${skills}.`, evidenceLine, "", "I’m reaching out proactively rather than assuming there is a specific opening. If you recruit for roles that fit my background, I’d be happy to share my resume and discuss relevant opportunities.", "", "Thank you,", name].join("\n"); }
