...
function serviceWithHtml(html: string, now?: Date) { return new ProactiveRecruiterDiscoveryService({ fetchText: async () => html, resolveEmployerDomain: domainResolver, ...(now ? { now: () => now } : {}) }); }
