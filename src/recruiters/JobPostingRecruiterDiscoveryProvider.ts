  ...
  try {
    for (const path of PUBLIC_PATHS) {
      ...
      try {
        ...
        // Public discovery is best-effort. One unavailable path must not block the others.
      }
    }
  } finally {
...
export class JobPostingRecruiterDiscoveryProvider implements RecruiterDiscoveryProvider {
  ...
  async discover(input: RecruiterDiscoveryInput): Promise<RecruiterDiscoveryResult> {
    ...
    for (const source of sources) {
      for (const email of extractRecruiterEmailsFromPublicText(source, input.companyDomain)) emails.add(email);
    }
