...
export class HunterRecruiterDiscoveryProvider implements RecruiterDiscoveryProvider {
  ...
  private readonly delegate: RecruiterDiscoveryProvider | null;
  ...
  constructor(options: HunterRecruiterDiscoveryProviderOptions) {
    ...
    const configuredProvider = process.env.RECRUITER_DISCOVERY_PROVIDER ?? "hunter";
    if (configuredProvider === "snov") {
    ...
    if (configuredProvider === "job-posting") {
      this.delegate = new JobPostingRecruiterDiscoveryProvider();
      this.name = this.delegate.name;
      return;
    }
    if (!this.apiKey) throw new Error("Hunter API key is required");
    this.delegate = null;
    this.name = "hunter";
  }
