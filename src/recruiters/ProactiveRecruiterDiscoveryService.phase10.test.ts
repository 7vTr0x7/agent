...
describe("Phase 10 recruiter hiring-evidence honesty", () => {
  ...
  it("does not turn recruiter identity into a current-hiring claim", async () => {
    const service = new ProactiveRecruiterDiscoveryService({
      fetchText: async () => "Jane Doe - Technical Recruiter at Example Corp. https://linkedin.com/in/jane-doe",
  ...
  it("classifies explicit current hiring evidence separately", async () => {
    const service = new ProactiveRecruiterDiscoveryService({
      fetchText: async () => "Jane Doe - Technical Recruiter at Example Corp. We are currently hiring React engineers. https://linkedin.com/in/jane-doe",
      now: () => new Date("2026-09-13T00:00:00Z"),
      maxQueries: 1
    });
    const results = await service.discover(profile);
