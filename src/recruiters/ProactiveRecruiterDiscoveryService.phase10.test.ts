...
describe("Phase 10 recruiter hiring-evidence honesty", () => {
  ...
  it("does not turn recruiter identity into a current-hiring claim", async () => {
    const service = new ProactiveRecruiterDiscoveryService({ fetchText: async () => "<html><head><title>Jane Doe | Technical Recruiter | Example Corp</title></head><body><h1>Jane Doe</h1><p>Technical Recruiter at Example Corp. React frontend recruiting professional.</p><a href=\"https://linkedin.com/in/jane-doe\">LinkedIn</a></body></html>", now: () => new Date("2026-09-13T00:00:00Z"), maxQueries: 1 });
