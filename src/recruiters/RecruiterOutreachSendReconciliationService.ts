...
export class RecruiterOutreachSendReconciliationService {
 ...
 async runOnce():Promise<RecruiterOutreachReconciliationResult>{
  ...
  for(const stale of result.rows){
    ...
    for(const gmailMessageId of ids){const message=await this.mailbox.getMessage(gmailMessageId);if(message.rfcMessageId===deterministicMessageId(stale.id)){sentMessages.set(stale.id,{gmailMessageId:message.gmailMessageId,gmailThreadId:message.gmailThreadId});break;}}
  }
  ...
  for(const stale of result.rows){
    ...
    const requeuedResult=await this.database.query<{id:string}>(`UPDATE recruiter_outreach_messages SET status='PREPARED',send_claimed_at=NULL,failure_reason='Ambiguous Gmail send not found by deterministic Message-ID reconciliation; safely requeued.',updated_at=NOW() WHERE id=$1 AND status='SENDING' RETURNING id`,[stale.id]);
    if(requeuedResult.rows[0]){
      const taskId=cryptoRandomId();
      await this.database.query(`INSERT INTO tasks (id,task_type,payload,priority,available_at,max_attempts,dedupe_key,lease_expires_at) VALUES ($1,'SEND_RECRUITER_EMAIL',$2::jsonb,30,NOW(),3,$3,NULL) ON CONFLICT (dedupe_key) WHERE dedupe_key IS NOT NULL AND status IN ('PENDING','RUNNING') DO UPDATE SET updated_at=NOW()`,[taskId,JSON.stringify({messageId:stale.id,companyDomain:stale.companyDomain}),`recruiter-email-send:${stale.id}`]);
      requeued+=1;
    } else unresolved+=1;
  }
...
function cryptoRandomId():string{return `reconcile-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,14)}`;}
