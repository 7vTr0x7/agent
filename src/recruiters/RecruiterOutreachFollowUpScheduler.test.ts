...
describe("RecruiterOutreachFollowUpScheduler", () => {
  ...
  it("prepares due follow-ups and queues each message", async () => {
    const messages: PreparedRecruiterFollowUpMessage[] = [message, { ...message, id: "message-2", sequenceId: "sequence-2", sequenceStep: 2, recipientEmail: "talent@beta.dev", companyDomain: "beta.dev" }];
  ...
  it("does nothing when follow-ups are disabled", async () => {
    const repository = { prepareDueRecruiterFollowUps: jest.fn() } as unknown as RecruiterDiscoveryRepository;
    const sendDispatcher = { enqueue: jest.fn() } as unknown as RecruiterOutreachSendTaskDispatcher;
    const scheduler = new RecruiterOutreachFollowUpScheduler(repository, sendDispatcher, false);
    await expect(scheduler.runOnce()).resolves.toEqual({ prepared: 0, queued: 0, failed: 0 });
    expect(repository.prepareDueRecruiterFollowUps).not.toHaveBeenCalled();
    expect(sendDispatcher.enqueue).not.toHaveBeenCalled();
  });

  it("continues queueing remaining follow-ups when one queue operation fails", async () => {
    const secondMessage = { ...message, id: "message-2", sequenceId: "sequence-2", recipientEmail: "talent@beta.dev", companyDomain: "beta.dev" };
