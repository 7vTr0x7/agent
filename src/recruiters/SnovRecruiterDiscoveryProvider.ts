...
import { CANONICAL_MAILBOX_VERIFICATION_STATUS } from "./RecruiterMailboxVerification";
...
function normalizeSnovStatus(status: string | undefined): { verified: boolean; verificationStatus: string } {
  const normalized = status?.trim().toLowerCase() ?? "unknown";
  if (normalized === "valid") return { verified: true, verificationStatus: CANONICAL_MAILBOX_VERIFICATION_STATUS };
  if (normalized === "not_valid") return { verified: false, verificationStatus: "INVALID" };
  return { verified: false, verificationStatus: normalized || "UNVERIFIED" };
}
...
export class SnovRecruiterDiscoveryProvider implements RecruiterDiscoveryProvider {
  ...
  async discover(input: RecruiterDiscoveryInput): Promise<RecruiterDiscoveryResult> { const domain = normalizeDomain(input.companyDomain); if (!domain) throw new Error("A valid company domain is required for Snov discovery"); const startUrl = new URL(`${SNOV_API_BASE}/v2/domain-search/prospects/start`); startUrl.searchParams.set("domain", domain); startUrl.searchParams.set("page", "1"); for (const position of RECRUITER_POSITIONS) startUrl.searchParams.append("positions[]", position); const started = await this.request<SnovStartResponse>(startUrl.toString(), { method: "POST" }); const taskHash = started.data?.task_hash; if (!taskHash) throw new Error("Snov prospect discovery returned no task hash"); const prospects = await this.poll<SnovProspectResult>(`${SNOV_API_BASE}/v2/domain-search/prospects/result/${taskHash}`); const contacts: RecruiterContactCandidate[] = []; for (const prospect of prospects.data ?? []) { if (!isRecruiterPosition(prospect.position) || !prospect.search_emails_start) continue; const emailStart = await this.request<SnovStartResponse>(prospect.search_emails_start, { method: "POST" }); const emailTask = emailStart.data?.task_hash; if (!emailTask) continue; const emails = await this.poll<SnovEmailResult>(`${prospect.search_emails_start}/result/${emailTask}`); for (const item of emails.data ?? []) { if (!item.email) continue; const email = normalizeEmail(item.email); if (!validEmail(email) || email.split("@").pop() !== domain) continue; const normalized = normalizeSnovStatus(item.smtp_status); const verificationEvidence = normalized.verified ? [{ provider: "snov", status: normalized.verificationStatus, mailboxLevel: true, source: "snov_prospect_email_verification" }] : undefined; contacts.push({ email, fullName: [prospect.first_name, prospect.last_name].filter(Boolean).join(" ") || undefined, title: prospect.position, confidence: normalized.verified ? 95 : 70, verified: normalized.verified, verificationStatus: normalized.verificationStatus, verificationEvidence, provider: this.name, sources: [{ url: prospect.source_page, type: "snov_prospect" }] }); } } return { provider: this.name, contacts, discoveredAt: new Date() }; }
  async verify(email: string): Promise<RecruiterVerificationResult> { const normalized = normalizeEmail(email); if (!validEmail(normalized)) return { email: normalized, verified: false, status: "INVALID", confidence: 0, verificationEvidence: [{ provider: "snov", status: "INVALID", confidence: 0, mailboxLevel: false, source: "snov_email_verification" }] }; const form = new URLSearchParams(); form.append("emails[]", normalized); const started = await this.request<SnovStartResponse>(`${SNOV_API_BASE}/v2/email-verification/start`, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: form }); const taskHash = started.data?.task_hash; if (!taskHash) return { email: normalized, verified: false, status: "UNVERIFIED", verificationEvidence: [{ provider: "snov", status: "UNVERIFIED", mailboxLevel: false, source: "snov_email_verification" }] }; const result = await this.poll<SnovVerificationResult>(`${SNOV_API_BASE}/v2/email-verification/result?task_hash=${encodeURIComponent(taskHash)}`); const match = result.data?.find((item) => normalizeEmail(item.email ?? "") === normalized); const providerStatus = match?.status ?? match?.smtp_status ?? "unknown"; const normalizedStatus = normalizeSnovStatus(providerStatus); return { email: normalized, verified: normalizedStatus.verified, status: normalizedStatus.verificationStatus, confidence: match?.score, verificationEvidence: [{ provider: "snov", status: normalizedStatus.verificationStatus, confidence: match?.score, mailboxLevel: normalizedStatus.verificationStatus === "mailbox_verified", source: "snov_email_verification" }] }; }
}

export { normalizeSnovStatus };
