...
describe("RecruiterOutreachSendReconciliationService",()=>{
 it("reconciles a stale SENDING message only when its deterministic Message-ID is found",async()=>{
  ...
  const database={query:jest.fn().mockResolvedValue({rows:[{id:messageId,recipientEmail:"recruiter@example.com",subject:"Frontend Engineer",sendClaimedAt:new Date(),companyDomain:"example.com"}]})} as unknown as Database;
