...
function identity(url:string,text:string,domain:string):RecruiterIdentityCandidate|null { const i=text.toLowerCase().indexOf(url.toLowerCase()),snippet=text.slice(Math.max(0,i-360),Math.min(text.length,i+700)); if(!RECRUITING.test(snippet)||NON_RECRUITING.test(snippet))return null; const m=snippet.match(/([A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){1,4})\s*(?:-|•|\||:)\s*([^|•]{3,120})/),name=m?.[1]?.trim(); if(!name)return null;const words=name.split(/\s+/);if(words.length<2||words.length>5||!words.every(w=>/^[A-Z][A-Za-z.'-]+$/.test(w)))return null; return {fullName:name,title:m?.[2]?.trim(),department:"recruiting",confidence:90,verified:false,verificationStatus:"identity_public_source",provider:"public-web",companyDomain:domain,recruitingContext:m?.[2]?.trim()||snippet.slice(0,300),discoveryEvidence:[snippet.slice(0,700)],discoveredAt:new Date(),linkedinProfileUrl:url,sources:[{url,type:"public_linkedin_search",confidence:90}]}; }
