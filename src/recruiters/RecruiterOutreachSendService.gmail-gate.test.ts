...
describe("RecruiterOutreachSendService Gmail kill switch", () => {
  const controlled = { controlledMessageId: message.id, controlledRecipient: message.recipientEmail };
  it("reaches the Gmail kill switch only after explicit Phase 6 confirmation", async () => {
    const mail = { sendMessage: jest.fn() };
    const service = new RecruiterOutreachSendService({ repository: repository(), mailbox: mail as never, dryRun: false, outboundEnabled: true, gmailEnabled: false, activation: "canary", controlledSendConfirmation: CONTROLLED_SEND_CONFIRMATION, ...controlled, maxMessagesPerDay: 1, maxMessagesPerHour: 1 });
