...
async function main(): Promise<void> {
  ...
  if (!config.automationEnabled) {
    ...
    if (proactiveRuntime) {
      handlers.set(PROACTIVE_RECRUITER_DISCOVERY_TASK, proactiveRuntime.handler);
      handlers.set(PROACTIVE_RECRUITER_OUTREACH_TASK, proactiveRuntime.handler);
      handlers.set(SEND_RECRUITER_EMAIL_TASK, proactiveRuntime.sendHandler);
    }
    ...
    const proactiveRecruiterLoop = (): Promise<void> => runPeriodicLoop({ name: "proactive-recruiter-discovery", intervalMs: config.proactiveRecruiter.intervalMs, signal: shutdownController.signal, logger, sleep, runOnce: async () => { if (!proactiveRuntime || !config.proactiveRecruiter.enabled) return; await proactiveRuntime.dispatcher.enqueueDiscovery({ candidateProfileId: candidateProfile.id, candidateName: candidateProfile.fullName ?? ([candidateProfile.firstName, candidateProfile.lastName].filter(Boolean).join(" ") || undefined), yearsExperience: candidateProfile.yearsExperience, skills: [...candidateProfile.skills], targetRoles: [...candidateProfile.targetTitles], location: candidateProfile.location, preferredLocations: csvEnvironment("CANDIDATE_PREFERRED_LOCATIONS"), remoteEligible: booleanEnvironment("CANDIDATE_REMOTE_ELIGIBLE", true), maxCandidates: config.proactiveRecruiter.maxCandidatesPerRun }); } });
    const loops: Array<Promise<void>> = [];
    if (discoveryRuntime) loops.push(discoveryLoop());
    if (proactiveRuntime) loops.push(proactiveRecruiterLoop());
    await Promise.all([discoveryWorker.run(), ...loops]);
  ...
  const submissionService = new ApplicationSubmissionService(browserSessions, adaptersRegistry, applicationRepository, undefined, undefined, undefined, undefined, undefined, undefined, config.applicationDryRun, undefined, undefined, undefined, config.applicationLiveEnabled);
  ...
  if (config.recruiterOutreach.enabled) {
    ...
    if (config.proactiveRecruiter.enabled) {
      proactiveRecruiterDispatcher = new ProactiveRecruiterTaskDispatcher(taskQueue);
      proactiveRecruiterHandler = new ProactiveRecruiterTaskHandler(new ProactiveRecruiterDiscoveryService(), new ProactiveRecruiterRepository(database), recruiterSendDispatcher, { enabled: config.proactiveRecruiter.enabled, sendEnabled: config.proactiveRecruiter.sendEnabled && config.gmail.enabled && config.outboundEnabled, maxCandidatesPerRun: config.proactiveRecruiter.maxCandidatesPerRun, requireVerifiedEmail: config.recruiterOutreach.requireVerifiedEmail }, logger);
    }
  }
  ...
  if (proactiveRecruiterHandler) { handlers.set(PROACTIVE_RECRUITER_DISCOVERY_TASK, proactiveRecruiterHandler); handlers.set(PROACTIVE_RECRUITER_OUTREACH_TASK, proactiveRecruiterHandler); }
