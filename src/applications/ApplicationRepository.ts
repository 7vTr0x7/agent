...
export class ApplicationRepository {
      ...
      if (!jobId) {
        ...
        if (!jobId) {
          const recovered = await client.query<{ id: string }>(
            `SELECT id FROM jobs
             WHERE job_opportunity_id = $1::uuid OR regexp_replace(trim(url), '[?#].*$', '') = $2
             ORDER BY CASE WHEN job_opportunity_id = $1::uuid THEN 0 ELSE 1 END, created_at ASC, id ASC LIMIT 1`,
    ...
    return this.database.transaction(async (client) => {
      const current = await client.query<{ status: string; candidate_profile_id: string; company_name: string }>(
        `SELECT a.status, a.candidate_profile_id, jo.company_name
         FROM applications a INNER JOIN job_opportunities jo ON jo.id = a.job_opportunity_id
      ...
      const submissionCount = await client.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM applications a
         WHERE a.candidate_profile_id = $1
           AND (a.status = 'SUBMISSION_IN_PROGRESS'
             OR (a.status = 'SENT' AND a.applied_at >= CURRENT_DATE)
             OR EXISTS (SELECT 1 FROM application_attempts aa WHERE aa.application_id = a.id AND aa.submitted = TRUE AND aa.attempted_at >= CURRENT_DATE))`,
      ...
      const attempt = await client.query<{ id: string }>(
        `INSERT INTO application_attempts (
           ...
           outcome, phase, idempotency_key, task_id, worker_id, target_url, metadata, updated_at
         ) VALUES (
           $1, NULL, TRUE, FALSE, 'Submission attempt reserved before external action.',
           NULL, NULL, NOW(), NULL, 'RESERVED', $2, $3, $4, $5, $6::jsonb, NOW()
         ) RETURNING id`,
        [applicationId, idempotencyKey, taskId, workerId, targetUrl, JSON.stringify({ dailySubmissionsUsed: submissionsUsed, companySubmissionsUsed })]
      );
      ...
      await client.query(
        ...
        [applicationId, row.status, JSON.stringify({ attemptId, idempotencyKey, taskId, workerId, dailySubmissionsUsed: submissionsUsed, companySubmissionsUsed })]
      );
    ...
    const result = await this.database.query(
      `UPDATE application_attempts
       SET phase = $2,
           ...
           updated_at = NOW()
       WHERE id = $1 AND phase IS DISTINCT FROM 'FINALIZED'`,
      [attemptId, phase, patch.submissionStartedAt ?? null, patch.requestSentAt ?? null, patch.responseReceivedAt ?? null, patch.confirmationAttemptedAt ?? null, patch.confirmationReceivedAt ?? null, patch.responseStatus ?? null, patch.finalUrl ?? null, patch.ambiguityReason ?? null, patch.metadata ? JSON.stringify(patch.metadata) : null]
    );
  ...
  async finalizeSubmissionAttempt(applicationId: string, attemptId: string, adapterName: string, result: NormalizedApplicationSubmissionResult): Promise<boolean> {
    return this.database.transaction(async (client) => {
      ...
      const attempt = await client.query<{ id: string; application_id: string; phase: string | null }>(
        `SELECT id, application_id, phase FROM application_attempts WHERE id = $1 FOR UPDATE`,
        [attemptId]
      );
      ...
      await client.query(
        `UPDATE application_attempts
         SET adapter_name = $2,
             ...
             reason = $4,
             confirmation_url = $5,
             external_application_id = $6,
             outcome = $7,
             phase = 'FINALIZED',
             final_url = $8,
             response_status = $9,
             request_sent_at = $10,
             response_received_at = $11,
             confirmation_attempted_at = $12,
             confirmation_received_at = $13,
             ambiguity_reason = $14,
      ...
      await client.query(
        `UPDATE applications
         SET status = $2,
             applied_at = CASE WHEN $2 = 'SENT' THEN NOW() ELSE applied_at END,
             updated_at = NOW()
         WHERE id = $1`,
  ...
  async listStaleSubmissions(olderThanMinutes: number): Promise<StaleSubmission[]> {
    ...
    const result = await this.database.query<{ id: string; candidate_profile_id: string; company_name: string; target_url: string; updated_at: Date }>(
      `SELECT a.id, a.candidate_profile_id, jo.company_name, jo.canonical_url AS target_url, a.updated_at
       FROM applications a INNER JOIN job_opportunities jo ON jo.id = a.job_opportunity_id
       WHERE a.status = 'SUBMISSION_IN_PROGRESS' AND a.updated_at < NOW() - ($1 * INTERVAL '1 minute')
  ...
  async listStaleSubmissionEvidence(olderThanMinutes: number): Promise<StaleSubmissionEvidence[]> {
    ...
    const result = await this.database.query<StaleSubmissionEvidenceRow>(
      `SELECT a.id, a.candidate_profile_id, jo.company_name, jo.canonical_url AS target_url, a.updated_at,
              aa.id AS attempt_id, aa.outcome AS attempt_outcome, aa.phase AS attempt_phase, aa.idempotency_key,
              aa.task_id, t.status AS task_status, t.lease_expires_at AS task_lease_expires_at,
              aa.confirmation_url, aa.external_application_id, aa.final_url, aa.response_status,
              aa.request_sent_at, aa.response_received_at, aa.confirmation_attempted_at,
              aa.confirmation_received_at, aa.ambiguity_reason, aa.metadata
       FROM applications a
       INNER JOIN job_opportunities jo ON jo.id = a.job_opportunity_id
       LEFT JOIN LATERAL (
         SELECT * FROM application_attempts aa0
         WHERE aa0.application_id = a.id
         ORDER BY aa0.attempted_at DESC, aa0.id DESC LIMIT 1
       ) aa ON TRUE
  ...
  async reconcileStaleSubmissions(olderThanMinutes: number): Promise<StaleReconciliationResult> {
    ...
    for (const submission of stale) {
      ...
      if (submission.attemptOutcome === "CONFIRMED_SUCCESS") {
        if (await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "CONFIRMED_SUCCESS")) result.confirmedSuccess += 1;
        continue;
      }
      if (submission.attemptOutcome === "DEFINITIVE_FAILURE") {
        if (await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "DEFINITIVE_FAILURE", submission.ambiguityReason ?? "Adapter reported a definitive submission failure.")) result.definitiveFailure += 1;
        continue;
      }
      if (submission.attemptOutcome === "NOT_SUBMITTED" && submission.attemptPhase === "FINALIZED") {
        if (await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "NOT_SUBMITTED")) result.safeToRetry += 1;
        continue;
      }

      if (submission.attemptOutcome === null && submission.attemptPhase === "RESERVED") {
        if (await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "NOT_SUBMITTED", "Task ended before the submission attempt entered external execution; safe to retry.")) result.safeToRetry += 1;
        continue;
      }
