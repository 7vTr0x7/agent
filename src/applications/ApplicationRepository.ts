...
import {
  ApplicationSubmissionOutcome,
  ApplicationSubmissionEvidence,
...
export interface StaleSubmissionEvidence extends StaleSubmission {
  ...
  attemptOutcome: ApplicationSubmissionOutcome | null;
...
export class ApplicationRepository {
    ...
    return this.database.transaction(async (client) => {
      const candidate = await client.query<{
        ...
        `SELECT
           ...
           jo.updated_at,
           EXISTS (
             SELECT 1 FROM job_rankings jr
             WHERE jr.job_opportunity_id = jo.id
               AND jr.candidate_profile_id = md.candidate_profile_id
           ) AS has_ranking,
           existing_application.id AS existing_application_id,
           existing_application.status AS existing_application_status,
      ...
      const reusableExistingApplication =
        row.existing_application_id !== null &&
        ["READY", "DRAFTED"].includes(row.existing_application_status ?? "");
      ...
      const policy = evaluateApplicationPolicy({
        ...
        hasExistingApplication: row.has_application && !reusableExistingApplication,
      ...
      if (!jobId) {
        ...
        if (!jobId) {
          const recovered = await client.query<{ id: string }>(
            `SELECT id
             FROM jobs
             WHERE job_opportunity_id = $1::uuid
                OR regexp_replace(trim(url), '[?#].*$', '') = $2
             ORDER BY CASE WHEN job_opportunity_id = $1::uuid THEN 0 ELSE 1 END,
                      created_at ASC, id ASC
             LIMIT 1`,
            [jobOpportunityId, row.canonical_url]
          );
      ...
      if (reusableExistingApplication && row.existing_application_id) {
        return {
          prepared: true,
          application: {
            applicationId: row.existing_application_id,
            jobOpportunityId,
            candidateProfileId,
      ...
      const inserted = await client.query<{ id: string }>(
        `INSERT INTO applications (job_id, job_opportunity_id, candidate_profile_id, status)
         VALUES ($1, $2, $3, 'READY')
         ON CONFLICT (job_opportunity_id) DO NOTHING
         RETURNING id`,
        [jobId, jobOpportunityId, candidateProfileId]
      );
      ...
      await client.query(
        `INSERT INTO application_events
          (application_id, from_status, to_status, event_type, metadata)
         VALUES ($1, NULL, 'READY', 'APPLICATION_PREPARED', $2::jsonb)`,
        [application.id, JSON.stringify({ jobOpportunityId, candidateProfileId })]
      );
  ...
  async beginSubmissionAttempt(
    ...
    return this.database.transaction(async (client) => {
      ...
      const excluded = [...PERMANENTLY_EXCLUDED_COMPANIES, ...this.excludedCompanies].some(
        (name) => name.trim().toLowerCase() === companyKey
      );
      ...
      await client.query(
        `UPDATE applications
         SET status = 'SUBMISSION_IN_PROGRESS', updated_at = NOW()
         WHERE id = $1`,
        [applicationId]
      );
      ...
      const attempt = await client.query<{ id: string }>(
        `INSERT INTO application_attempts (
           application_id, adapter_name, safety_allowed, submitted, reason,
           confirmation_url, external_application_id, attempted_at,
           outcome, phase, idempotency_key, task_id, worker_id, target_url,
           metadata, updated_at
         ) VALUES (
           $1, NULL, TRUE, FALSE, 'Submission attempt reserved before external action.',
           NULL, NULL, NOW(), NULL, 'RESERVED', $2, $3, $4, $5, $6::jsonb, NOW()
         )
         RETURNING id`,
        [
          applicationId,
          ...
          workerId,
          targetUrl,
          JSON.stringify({ dailySubmissionsUsed: submissionsUsed, companySubmissionsUsed })
        ]
      );
      ...
      await client.query(
        ...
        [
          applicationId,
          row.status,
          JSON.stringify({ attemptId, idempotencyKey, taskId, workerId, dailySubmissionsUsed: submissionsUsed, companySubmissionsUsed })
        ]
      );
  ...
  async beginSubmission(applicationId: string): Promise<boolean> {
    return Boolean(await this.beginSubmissionAttempt(applicationId));
  }
  ...
  async updateSubmissionAttemptPhase(
    ...
    const result = await this.database.query(
      `UPDATE application_attempts
       SET phase = $2,
           submission_started_at = COALESCE($3::timestamptz, submission_started_at),
           request_sent_at = COALESCE($4::timestamptz, request_sent_at),
           response_received_at = COALESCE($5::timestamptz, response_received_at),
           confirmation_attempted_at = COALESCE($6::timestamptz, confirmation_attempted_at),
           confirmation_received_at = COALESCE($7::timestamptz, confirmation_received_at),
           response_status = COALESCE($8::integer, response_status),
           final_url = COALESCE($9::text, final_url),
           ambiguity_reason = COALESCE($10::text, ambiguity_reason),
           metadata = CASE WHEN $11::jsonb IS NULL THEN metadata ELSE COALESCE(metadata, '{}'::jsonb) || $11::jsonb END,
           updated_at = NOW()
       WHERE id = $1
         AND phase IS DISTINCT FROM 'FINALIZED'`,
      [
        attemptId,
        phase,
        patch.submissionStartedAt ?? null,
        patch.requestSentAt ?? null,
        patch.responseReceivedAt ?? null,
        patch.confirmationAttemptedAt ?? null,
        patch.confirmationReceivedAt ?? null,
        patch.responseStatus ?? null,
        patch.finalUrl ?? null,
        patch.ambiguityReason ?? null,
  ...
  async finalizeSubmissionAttempt(
    applicationId: string,
    attemptId: string,
    result: NormalizedApplicationSubmissionResult
  ): Promise<boolean> {
    return this.database.transaction(async (client) => {
      ...
      if (!application || application.status !== "SUBMISSION_IN_PROGRESS") return false;

      const attempt = await client.query<{ id: string; application_id: string; phase: string | null }>(
        `SELECT id, application_id, phase
      ...
      if (confirmed && !result.confirmationUrl?.trim() && !result.externalApplicationId?.trim()) {
        throw new Error("Confirmed application outcome requires confirmation evidence.");
      }

      const nextStatus =
        outcome === "CONFIRMED_SUCCESS" ? "SENT" :
        outcome === "DEFINITIVE_FAILURE" ? "SUBMISSION_FAILED" :
        outcome === "NOT_SUBMITTED" ? "READY" :
        "SUBMISSION_UNKNOWN";

      const eventType =
        outcome === "CONFIRMED_SUCCESS" ? "APPLICATION_SUBMITTED" :
        outcome === "DEFINITIVE_FAILURE" ? "APPLICATION_SUBMISSION_FAILED" :
        outcome === "NOT_SUBMITTED" ? "APPLICATION_SUBMISSION_NOT_SUBMITTED" :
        "APPLICATION_SUBMISSION_AMBIGUOUS";
      ...
      await client.query(
        `UPDATE application_attempts
         SET adapter_name = $2,
             ...
             confirmation_attempted_at = $13,
             confirmation_received_at = $14,
             ambiguity_reason = $15,
        ...
        [
          attemptId,
          result.evidence.finalUrl ? result.evidence.finalUrl : null,
          true,
          confirmed,
          result.reason,
          ...
          outcome,
          result.evidence.finalUrl,
          result.evidence.responseStatus,
          result.evidence.requestSentAt,
          result.evidence.responseReceivedAt,
          new Date(),
          confirmed ? new Date() : null,
          outcome === "AMBIGUOUS" ? result.reason : null,
          JSON.stringify({
            requestObserved: result.evidence.requestObserved,
            responseObserved: result.evidence.responseObserved,
      ...
      await client.query(
        `INSERT INTO application_events
          (application_id, from_status, to_status, event_type, metadata)
         VALUES ($1, 'SUBMISSION_IN_PROGRESS', $2, $3, $4::jsonb)`,
        [
          applicationId,
          nextStatus,
          eventType,
          JSON.stringify({
            ...
            evidence: {
              requestObserved: result.evidence.requestObserved,
              responseObserved: result.evidence.responseObserved,
              responseStatus: result.evidence.responseStatus,
  ...
  async listStaleSubmissionEvidence(olderThanMinutes: number): Promise<StaleSubmissionEvidence[]> {
    ...
    const result = await this.database.query<StaleSubmissionEvidenceRow>(
      `SELECT
         a.id,
         ...
         a.updated_at,
         aa.id AS attempt_id,
         aa.outcome AS attempt_outcome,
         aa.phase AS attempt_phase,
         aa.idempotency_key,
         aa.task_id,
         t.status AS task_status,
         t.lease_expires_at AS task_lease_expires_at,
         aa.confirmation_url,
         aa.external_application_id,
         aa.final_url,
         aa.response_status,
         aa.request_sent_at,
         aa.response_received_at,
         aa.confirmation_attempted_at,
         aa.confirmation_received_at,
         aa.ambiguity_reason,
         aa.metadata
       FROM applications a
       INNER JOIN job_opportunities jo ON jo.id = a.job_opportunity_id
       LEFT JOIN LATERAL (
         SELECT *
         FROM application_attempts aa0
         WHERE aa0.application_id = a.id
         ORDER BY aa0.attempted_at DESC, aa0.id DESC
         LIMIT 1
       ) aa ON TRUE
  ...
  async reconcileStaleSubmissions(olderThanMinutes: number): Promise<StaleReconciliationResult> {
    ...
    for (const submission of stale) {
      const activeTask = submission.taskStatus === "RUNNING" &&
        submission.taskLeaseExpiresAt !== null &&
        submission.taskLeaseExpiresAt.getTime() > Date.now();

      if (activeTask) {
        result.unchangedActive += 1;
        continue;
      }

      if (submission.attemptOutcome === "CONFIRMED_SUCCESS") {
        const resolved = await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "CONFIRMED_SUCCESS");
        if (resolved) result.confirmedSuccess += 1;
        continue;
      }

      if (submission.attemptOutcome === "DEFINITIVE_FAILURE") {
        const resolved = await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "DEFINITIVE_FAILURE", submission.ambiguityReason ?? "Adapter reported a definitive submission failure.");
        if (resolved) result.definitiveFailure += 1;
        continue;
      }

      if (submission.attemptOutcome === "NOT_SUBMITTED" && submission.attemptPhase === "FINALIZED") {
        const resolved = await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "NOT_SUBMITTED");
        if (resolved) result.safeToRetry += 1;
        continue;
      }

      const safeReservedCrash =
        submission.attemptOutcome === null &&
        submission.attemptPhase === "RESERVED";

      if (safeReservedCrash) {
        const resolved = await this.resolvePersistedAttemptOutcome(submission.applicationId, submission.attemptId, "NOT_SUBMITTED");
        if (resolved) result.safeToRetry += 1;
        continue;
      }

      const markedUnknown = await this.markSubmissionUnknown(
        submission.applicationId,
        submission.attemptId,
        submission.ambiguityReason ??
          "Submission attempt became stale without definitive external confirmation; reconciliation is required and resubmission is blocked."
      );
      if (markedUnknown) result.markedUnknown += 1;
    }

    return result;
  }

  private async resolvePersistedAttemptOutcome(
    applicationId: string,
    attemptId: string | null,
    outcome: "CONFIRMED_SUCCESS" | "DEFINITIVE_FAILURE" | "NOT_SUBMITTED",
    reason?: string
  ): Promise<boolean> {
    return this.database.transaction(async (client) => {
      ...
      if (attemptId) {
        await client.query(
          `UPDATE application_attempts
           SET outcome = $2,
               submitted = ($2 = 'CONFIRMED_SUCCESS'),
               phase = 'FINALIZED',
               reason = COALESCE($3, reason),
               updated_at = NOW()
           WHERE id = $1`,
          [attemptId, outcome, reason ?? null]
        );
      }
      await client.query(
        `UPDATE applications
         SET status = $2,
             applied_at = CASE WHEN $2 = 'SENT' THEN COALESCE(applied_at, NOW()) ELSE applied_at END,
             updated_at = NOW()
         WHERE id = $1`,
        [applicationId, nextStatus]
      );
      await client.query(
        ...
        [
          applicationId,
          nextStatus,
          outcome === "CONFIRMED_SUCCESS" ? "APPLICATION_SUBMISSION_RECONCILED_SUCCESS" :
            outcome === "DEFINITIVE_FAILURE" ? "APPLICATION_SUBMISSION_RECONCILED_FAILURE" :
            "APPLICATION_SUBMISSION_RECONCILED_NOT_SUBMITTED",
          JSON.stringify({ attemptId, outcome, reason: reason ?? null })
        ]
      );
      return true;
    });
  ...
  private async markSubmissionUnknown(applicationId: string, attemptId: string | null, reason: string): Promise<boolean> {
    return this.database.transaction(async (client) => {
      ...
      await client.query(
        `UPDATE applications SET status = 'SUBMISSION_UNKNOWN', updated_at = NOW() WHERE id = $1`,
        [applicationId]
      );
      await client.query(
        `INSERT INTO application_events
          (application_id, from_status, to_status, event_type, metadata)
         VALUES ($1, 'SUBMISSION_IN_PROGRESS', 'SUBMISSION_UNKNOWN', 'APPLICATION_SUBMISSION_RECONCILIATION_REQUIRED', $2::jsonb)`,
        [applicationId, JSON.stringify({ attemptId, reason, resubmissionBlocked: true })]
      );
      return true;
    });
  }
  ...
  async recoverVerifiedSubmission(applicationId: string, olderThanMinutes: number, evidence: VerifiedSubmissionEvidence): Promise<SubmittedApplicationResult | null> {
    ...
    return this.database.transaction(async (client) => {
      const current = await client.query<{ status: string; updated_at: Date }>(
        `SELECT status, updated_at FROM applications WHERE id = $1 FOR UPDATE`,
        [applicationId]
      );
      const row = current.rows[0];
      if (!row) return null;
      ...
      const latestAttempt = await client.query<{ id: string }>(
        `SELECT id FROM application_attempts WHERE application_id = $1 ORDER BY attempted_at DESC, id DESC LIMIT 1`,
        [applicationId]
      );
      ...
      if (attemptId) {
        await client.query(
          `UPDATE application_attempts
           SET outcome = 'CONFIRMED_SUCCESS', submitted = TRUE, phase = 'FINALIZED',
               confirmation_url = $2, external_application_id = $3,
               confirmation_received_at = NOW(), updated_at = NOW()
           WHERE id = $1`,
          [attemptId, evidence.confirmationUrl, evidence.externalApplicationId]
        );
      }
      ...
      await client.query(
        `INSERT INTO application_events
          (application_id, from_status, to_status, event_type, metadata)
         VALUES ($1, $2, 'SENT', 'APPLICATION_SUBMISSION_RECOVERED', $3::jsonb)`,
        [applicationId, fromStatus, JSON.stringify({ ...evidence, attemptId })]
      );
  ...
  async resolveUnknownAsDefinitiveFailure(applicationId: string, reason: string, verificationSource: "INDEPENDENT_REJECTION"): Promise<boolean> {
    ...
    return this.database.transaction(async (client) => {
      ...
      if (attemptId) {
        await client.query(
          `UPDATE application_attempts
           SET outcome = 'DEFINITIVE_FAILURE', submitted = FALSE, phase = 'FINALIZED',
               reason = $2, ambiguity_reason = NULL,
               metadata = COALESCE(metadata, '{}'::jsonb) || $3::jsonb,
               updated_at = NOW()
           WHERE id = $1`,
          [attemptId, reason, JSON.stringify({ verificationSource })]
        );
      }
      ...
      await client.query(
        `INSERT INTO application_events
          (application_id, from_status, to_status, event_type, metadata)
         VALUES ($1, 'SUBMISSION_UNKNOWN', 'SUBMISSION_FAILED', 'APPLICATION_SUBMISSION_RECONCILED_FAILURE', $2::jsonb)`,
        [applicationId, JSON.stringify({ attemptId, reason, verificationSource })]
      );
      return true;
    });
  }
...
interface StaleSubmissionEvidenceRow {
  ...
  attempt_outcome: ApplicationSubmissionOutcome | null;
  ...
  confirmation_attempted_at: Date | null;
  confirmation_received_at: Date | null;
