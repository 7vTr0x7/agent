...
export class ApplicationRepository {
  constructor(
    ...
    private readonly rateLimitPolicy = new ApplicationRateLimitPolicy({ maxSubmissionsPerDay: 50 }),
    private readonly companyRateLimitPolicy = new ApplicationCompanyRateLimitPolicy({ maxSubmissionsPerCompanyPerDay: 5 })
  ) {}
  ...
  async prepare(jobOpportunityId: string, candidateProfileId: string): Promise<PrepareApplicationResult> {
    return this.database.transaction(async (client) => {
      const candidate = await client.query<{
        ...
        `
          SELECT jo.id AS job_opportunity_id, md.decision AS match_decision, jo.status AS opportunity_status,
            jo.title AS job_title, jo.company_name, jo.company_domain, jo.canonical_url, jo.description AS job_description,
            ...
            EXISTS (SELECT 1 FROM applications a WHERE a.job_opportunity_id = jo.id) AS has_application,
      ...
      const inserted = await client.query<{ id: string }>(
        `INSERT INTO applications (job_id, job_opportunity_id, status) VALUES ($1, $2, 'READY') ON CONFLICT (job_opportunity_id) DO NOTHING RETURNING id`,
      ...
      await client.query(
        `INSERT INTO application_events (application_id, from_status, to_status, event_type, metadata) VALUES ($1, NULL, 'READY', 'APPLICATION_PREPARED', $2::jsonb)`,
  ...
  async beginSubmission(applicationId: string): Promise<boolean> {
    return this.database.transaction(async (client) => {
      ...
      if (!row || !["READY", "DRAFTED"].includes(row.status)) return false;
      ...
      if (companySubmissionsUsed >= this.companyRateLimitPolicy.maxSubmissionsPerCompanyPerDay) return false;
  ...
  async recoverVerifiedSubmission(applicationId: string, olderThanMinutes: number, evidence: VerifiedSubmissionEvidence): Promise<SubmittedApplicationResult | null> {
    ...
    return this.database.transaction(async (client) => {
      ...
      await client.query(`UPDATE applications SET status = 'SENT', applied_at = NOW(), updated_at = NOW() WHERE id = $1`, [applicationId]);
      await client.query(`INSERT INTO application_events (application_id, from_status, to_status, event_type, metadata) VALUES ($1, 'SUBMISSION_IN_PROGRESS', 'SENT', 'APPLICATION_SUBMISSION_RECOVERED', $2::jsonb)`, [applicationId, JSON.stringify(evidence)]);
