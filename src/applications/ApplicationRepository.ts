...
export class ApplicationRepository {
  ...
  async beginSubmission(applicationId: string): Promise<boolean> {
    return this.database.transaction(async (client) => {
      ...
      await client.query(
        `UPDATE applications SET status = 'SUBMISSION_IN_PROGRESS', updated_at = NOW() WHERE id = $1`,
        [applicationId]
      );
      await client.query(
        `
        ...
        `,
        [applicationId, row.status, JSON.stringify({ dailySubmissionsUsed: submissionsUsed, companySubmissionsUsed })]
      );
      return true;
    });
  }
  ...
  async cancelSubmission(applicationId: string, reason: string): Promise<boolean> {
    return this.database.transaction(async (client) => {
      const updated = await client.query<{ status: string }>(
        `
          UPDATE applications SET status = 'READY', updated_at = NOW()
