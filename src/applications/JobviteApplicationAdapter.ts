...
const JOBVITE_HOST_PATTERNS = [
  /^jobs\.jobvite\.com$/i,
  /^careers\.jobvite\.com$/i,
  /^app\.jobvite\.com$/i
] as const;

const JOBVITE_SUBMIT_LABEL = /^(?:submit(?: application)?|apply(?: now)?|complete application)$/i;

export class JobviteSubmitControlResolver {
  async resolve(page: Page): Promise<Locator | null> {
    ...
    for (let index = 0; index < await controls.count(); index += 1) {
      ...
      const isKnownSubmitControl =
        ...
        dataTestId === "submit";
      const isSubmitLabel = JOBVITE_SUBMIT_LABEL.test(label);
...
export class JobviteApplicationAdapter implements ApplicationAdapter {
  readonly name = "jobvite";

  constructor(
    private readonly submitResolver = new JobviteSubmitControlResolver(),
  ...
  canHandle(url: string): boolean {
    try {
      ...
      return JOBVITE_HOST_PATTERNS.some((pattern) => pattern.test(hostname));
    } catch {
      return false;
    }
  }
  ...
  async submit(
    ...
    if (!submitControl) {
      return {
        ...
        reason:
          "Jobvite submit control could not be uniquely verified; manual review required."
      };
    }
    ...
    try {
      ...
      return {
        ...
        externalApplicationId: null,
        confirmationUrl: null,
        reason: `Jobvite submit control could not be clicked: ${
    ...
    if (!confirmation.confirmed) {
      return {
        ...
        confirmationUrl: null,
        reason:
          "Jobvite submit control was clicked, but application confirmation could not be verified."
      };
    }
    ...
    return {
      ...
      reason: confirmation.signal ?? "Jobvite application submission was confirmed."
    };
  }
}
