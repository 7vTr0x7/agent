...
const BAMBOOHR_HOST_PATTERNS = [
  /^.*\.bamboohr\.com$/i
] as const;

const BAMBOOHR_SUBMIT_LABEL = /^(?:submit(?: application)?|apply(?: now)?)$/i;

export class BambooHRSubmitControlResolver {
  async resolve(page: Page): Promise<Locator | null> {
    ...
    for (let index = 0; index < await controls.count(); index += 1) {
      ...
      const isKnownSubmitId =
        ...
        dataTestId === "submit" ||
        dataQa === "submit";
      const isSubmitLabel = BAMBOOHR_SUBMIT_LABEL.test(label);
...
export class BambooHRApplicationAdapter implements ApplicationAdapter {
  readonly name = "bamboohr";

  constructor(
    private readonly submitResolver = new BambooHRSubmitControlResolver(),
  ...
  canHandle(url: string): boolean {
    try {
      ...
      return BAMBOOHR_HOST_PATTERNS.some((pattern) => pattern.test(hostname));
    } catch {
      return false;
    }
  }
  ...
  async submit(
    ...
    if (!submitControl) {
      return {
        ...
        reason:
          "BambooHR submit control could not be uniquely verified; manual review required."
      };
    }
    ...
    try {
      ...
      return {
        ...
        externalApplicationId: null,
        confirmationUrl: null,
        reason: `BambooHR submit control could not be clicked: ${
    ...
    if (!confirmation.confirmed) {
      return {
        ...
        confirmationUrl: null,
        reason:
          "BambooHR submit control was clicked, but application confirmation could not be verified."
      };
    }
    ...
    return {
      ...
      reason: confirmation.signal ?? "BambooHR application submission was confirmed."
    };
  }
}
