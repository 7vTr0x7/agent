...
describe("WorkdayApplicationAdapter", () => {
  ...
  const context: ApplicationContext = {
    ...
    url: "https://company.myworkdayjobs.com/en-US/jobs/job-1"
  };

  it("recognizes Workday-hosted application URLs only", () => {
    const adapter = new WorkdayApplicationAdapter();

    expect(adapter.canHandle("https://company.myworkdayjobs.com/en-US/jobs/job-1")).toBe(true);
    expect(adapter.canHandle("https://wd5.myworkday.com/company/job/123")).toBe(true);
    expect(adapter.canHandle("https://example.workday.com/job/123")).toBe(true);
    expect(adapter.canHandle("https://example.com/jobs/123")).toBe(false);
    expect(adapter.canHandle("not-a-url")).toBe(false);
  });

  it("clicks one verified Workday submit control and requires confirmation", async () => {
    const adapter = new WorkdayApplicationAdapter();

    await page.setContent(`
      <form>
        <button type="submit" data-automation-id="submit">Submit Application</button>
      </form>
      <script>
