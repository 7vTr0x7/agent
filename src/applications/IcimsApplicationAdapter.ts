...
const ICIMS_HOST_PATTERNS = [
  /^careers\.icims\.com$/i,
  /^.*\.icims\.com$/i
] as const;

const ICIMS_SUBMIT_LABEL = /^(?:submit(?: application)?|complete application)$/i;

export class IcimsSubmitControlResolver {
  async resolve(page: Page): Promise<Locator | null> {
    ...
    for (let index = 0; index < await controls.count(); index += 1) {
      ...
      const isKnownSubmitId =
        ...
        dataTestId === "submit" ||
        dataQa === "submit";
      const isSubmitLabel = ICIMS_SUBMIT_LABEL.test(label);
...
export class IcimsApplicationAdapter implements ApplicationAdapter {
  readonly name = "icims";

  constructor(
    private readonly submitResolver = new IcimsSubmitControlResolver(),
  ...
  canHandle(url: string): boolean {
    try {
      ...
      return ICIMS_HOST_PATTERNS.some((pattern) => pattern.test(hostname));
    } catch {
      return false;
    }
  }
  ...
  async submit(
    ...
    if (!submitControl) {
      return {
        ...
        reason:
          "iCIMS submit control could not be uniquely verified; manual review required."
      };
    }
    ...
    try {
      ...
      return {
        ...
        externalApplicationId: null,
        confirmationUrl: null,
        reason: `iCIMS submit control could not be clicked: ${
    ...
    if (!confirmation.confirmed) {
      return {
        ...
        confirmationUrl: null,
        reason:
          "iCIMS submit control was clicked, but application confirmation could not be verified."
      };
    }
    ...
    return {
      ...
      reason: confirmation.signal ?? "iCIMS application submission was confirmed."
    };
  }
}
