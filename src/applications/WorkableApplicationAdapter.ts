...
const WORKABLE_HOST_PATTERNS = [
  /^apply\.workable\.com$/i,
  /^jobs\.workable\.com$/i,
  /^.*\.workable\.com$/i
] as const;

const WORKABLE_SUBMIT_LABEL = /^(?:submit(?: application)?|apply(?: now)?)$/i;

export class WorkableSubmitControlResolver {
  async resolve(page: Page): Promise<Locator | null> {
    ...
    for (let index = 0; index < await controls.count(); index += 1) {
      ...
      const isKnownSubmitId =
        ...
        dataTestId === "submit" ||
        dataQa === "submit";
      const isSubmitLabel = WORKABLE_SUBMIT_LABEL.test(label);
...
export class WorkableApplicationAdapter implements ApplicationAdapter {
  readonly name = "workable";

  constructor(
    private readonly submitResolver = new WorkableSubmitControlResolver(),
  ...
  canHandle(url: string): boolean {
    try {
      ...
      return WORKABLE_HOST_PATTERNS.some((pattern) => pattern.test(hostname));
    } catch {
      return false;
    }
  }
  ...
  async submit(
    ...
    if (!submitControl) {
      return {
        ...
        reason:
          "Workable submit control could not be uniquely verified; manual review required."
      };
    }
    ...
    try {
      ...
      return {
        ...
        externalApplicationId: null,
        confirmationUrl: null,
        reason: `Workable submit control could not be clicked: ${
    ...
    if (!confirmation.confirmed) {
      return {
        ...
        confirmationUrl: null,
        reason:
          "Workable submit control was clicked, but application confirmation could not be verified."
      };
    }
    ...
    return {
      ...
      reason: confirmation.signal ?? "Workable application submission was confirmed."
    };
  }
}
