...
interface AshbyJob { id: string; title: string; descriptionPlain?: string; descriptionHtml?: string; publishedAt?: string; jobUrl?: string; location?: string; workplaceType?: string; employmentType?: string; }
...
export class AshbyJobSource implements JobSource {
  ...
  async fetchJobs(): Promise<Job[]> {
    ...
    if (!response.ok) throw new AppError(`Ashby request failed: ${response.status}`, { code: "JOB_SOURCE_REQUEST_FAILED", statusCode: 502 });
  ...
  private normalize(job: AshbyJob): Job {
    ...
    if (!job.id || !job.title?.trim() || !description || !url) throw new AppError("Ashby returned an incomplete job posting", { code: "JOB_SOURCE_INVALID_DATA", statusCode: 502 });
    ...
    return {
      source: this.name, sourceJobId: job.id, url, title: job.title.trim(), companyName: this.jobBoardName,
      ...(this.companyDomain ? { companyDomain: this.companyDomain } : {}),
      location, country: inferCountry(location), workplaceType, employmentType: job.employmentType?.trim() || null,
      description, postedAt: job.publishedAt ? new Date(job.publishedAt) : null, updatedAt: null, contentHash
    };
  }
}
function normalizeWorkplaceType(value: string | undefined, location: string | null, description: string): Job["workplaceType"] { const explicit = value?.toLowerCase(); if (explicit === "remote" || explicit === "hybrid" || explicit === "onsite") return explicit; const text = `${location ?? ""} ${description}`.toLowerCase(); if (/\bremote\b|work from home|wfh/.test(text)) return "remote"; if (/\bhybrid\b/.test(text)) return "hybrid"; if (location) return "onsite"; return null; }
