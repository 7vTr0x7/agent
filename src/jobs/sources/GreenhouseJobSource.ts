...
export class GreenhouseJobSource implements JobSource {
  ...
  async fetchJobs(): Promise<Job[]> {
    ...
    if (!response.ok) {
      throw new AppError(`Greenhouse request failed: ${response.status}`, {
  ...
  private normalize(job: GreenhouseJob): Job {
    ...
    if (!job.id || !job.title?.trim() || !description || !url) {
      throw new AppError("Greenhouse returned an incomplete job posting", {
    ...
    return {
      ...
      companyName: this.boardToken,
      location,
      country: inferCountry(location),
      workplaceType: inferWorkplaceType(location, description),
      employmentType: null,
      description,
      postedAt: null,
...
function inferWorkplaceType(location: string | null, description: string): Job["workplaceType"] {
