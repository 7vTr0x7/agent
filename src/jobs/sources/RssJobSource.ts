...
export class RssJobSource implements JobSource {
  ...
  private normalize(item: RssItem): Job {
    ...
    const companyName =
      ...
      this.options.defaultCompanyName?.trim() ||
      "Unknown";
...
function parseRssItems(xml: string): RssItem[] {
  ...
  return blocks.flatMap((block, index) => {
    ...
    const guid = decodeXml(
      readTag(block, "guid") ?? readTag(block, "id") ?? link
    ).trim();
    const creator =
      decodeXml(
        readTag(block, "dc:creator") ?? readTag(block, "creator") ?? ""
      ).trim() || null;
    const publishedRaw =
    ...
    const publishedAt = publishedRaw
      ? new Date(decodeXml(publishedRaw).trim())
      : null;
    const location =
      decodeXml(readTag(block, "location") ?? "").trim() || null;
...
function inferCountry(location: string | null): string | null {
  ...
  if (
    /india|bangalore|bengaluru|mumbai|pune|hyderabad|chennai|delhi|gurugram|noida/.test(
      normalized
    )
  ) {
    return "India";
