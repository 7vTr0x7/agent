...
export class LeverJobSource implements JobSource {
  ...
  async fetchJobs(): Promise<Job[]> {
    ...
    return postings
      .filter((posting) => posting.state === undefined || posting.state === "published")
      .map((posting) => this.normalize(posting));
  }

  private normalize(posting: LeverPosting): Job {
    const description = posting.content?.description?.trim() ?? "";

    if (!posting.id || !posting.text || !description || !posting.urls?.show) {
      throw new AppError("Lever returned an incomplete job posting", {
