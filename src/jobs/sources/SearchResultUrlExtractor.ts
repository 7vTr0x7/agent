export interface SearchResultExtractionDiagnostics {
  readonly markdownCandidates: number;
  readonly hrefCandidates: number;
  readonly rssCandidates: number;
  readonly bareUrlCandidates: number;
  readonly redirectCandidates: number;
  readonly validCandidates: number;
  readonly normalizedUrls: number;
...
export function extractSearchResultUrls(page: string, baseUrl?: string): SearchResultExtractionResult {
  ...
  for (const match of page.matchAll(/\[[^\]]*\]\(([^)\s]+)\)/gi)) {
    candidates.push({ value: match[1] ?? "", kind: "markdown" });
  }

  for (const match of page.matchAll(/<link\b[^>]*>([\s\S]*?)<\/link>/gi)) {
    candidates.push({ value: match[1] ?? "", kind: "rss" });
  }
  for (const match of page.matchAll(/<link\b[^>]*\bhref\s*=\s*["']([^"']+)["'][^>]*>/gi)) {
    candidates.push({ value: match[1] ?? "", kind: "rss" });
  }
  for (const match of page.matchAll(/href\s*=\s*["']([^"']+)["']/gi)) {
    candidates.push({ value: match[1] ?? "", kind: "href" });
  }

  const textWithoutMarkup = page.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ");
  for (const match of textWithoutMarkup.matchAll(URL_RE)) {
    candidates.push({ value: match[0] ?? "", kind: "bare" });
  }
  ...
  for (const candidate of candidates) {
    ...
    if (!unwrapped.value) {
      reject(metrics, unwrapped.reason ?? "invalid_url");
      continue;
    }

    const normalized = normalizeCandidateUrl(unwrapped.value, baseUrl);
    if (!normalized) {
      reject(metrics, "rejected_candidate");
      continue;
    }
...
function reject(metrics: SearchResultExtractionDiagnostics, reason: string): void {
...
function unwrapDestination(value: string, baseUrl?: string): { value: string; redirect: boolean; reason?: string } {
  ...
  if (genericDestination && isLikelyTrackingWrapper(url)) {
    return { value: boundedDecode(genericDestination), redirect: true };
  }
...
function normalizeCandidateUrl(value: string, baseUrl?: string): string {
  ...
  const host = hostnameWithoutWww(url.hostname);
  if (isSearchEngineHost(host)) return "";
  if (isLocalHost(host)) return "";
  if (LOGIN_SEGMENT_RE.test(url.pathname)) return "";
...
function decodeBingDestination(value: string): string {
  ...
  if (/^a1/i.test(decoded)) {
    try {
      const base64 = decoded.slice(2).replace(/-/g, "+").replace(/_/g, "/");
      return boundedDecode(Buffer.from(base64, "base64").toString("utf8"));
    } catch { return ""; }
  }
  try {
    const base64 = decoded.replace(/-/g, "+").replace(/_/g, "/");
    const maybeUrl = Buffer.from(base64, "base64").toString("utf8");
    return /^https?:\/\//i.test(maybeUrl) ? maybeUrl : "";
  } catch { return ""; }
}

function decodeJinaDestination(url: URL): string {
  const path = url.pathname.replace(/^\/+/, "");
  if (/^https?:\/\//i.test(path)) return boundedDecode(path);
  return "";
}

function isLikelyTrackingWrapper(url: URL): boolean {
  const path = url.pathname.toLowerCase();
  return path.includes("redirect") || path.includes("out") || path.includes("click") || path.includes("track") || path === "/url" || path === "/l/";
}
...
function decodeHtmlEntities(value: string): string {
  ...
  return value
    ...
    .replace(/&([a-z][a-z0-9]+);/gi, (full: string, name: string) => named[name.toLowerCase()] ?? full);
}
