...
describe("SearchResultUrlExtractor", () => {
  ...
  it("extracts multiple markdown URLs", () => {
    expect(extract("[A](https://a.example/jobs/1) [B](https://b.example/careers/2)"))
      .toEqual(["https://a.example/jobs/1", "https://b.example/careers/2"]);
  });

  it("extracts HTML href values including single quotes", () => {
    expect(extract('<a href="https://example.com/jobs/1">A</a><a href=\'https://example.com/jobs/2\'>B</a>'))
      .toEqual(["https://example.com/jobs/1", "https://example.com/jobs/2"]);
  });

  it("resolves relative hrefs against the search page base URL", () => {
    expect(extract('<a href="/jobs/123">React Developer</a>', "https://example.com/search?q=react"))
      .toEqual(["https://example.com/jobs/123"]);
  });

  it("extracts RSS/XML links", () => {
    expect(extract("<item><link>https://example.com/jobs/123</link></item><link href=\"https://example.com/jobs/456\" />"))
      .toEqual(["https://example.com/jobs/123", "https://example.com/jobs/456"]);
  });

  it("extracts bare HTTPS URLs", () => {
    expect(extract("Result: https://example.com/careers/frontend-engineer"))
      .toEqual(["https://example.com/careers/frontend-engineer"]);
  });

  it("unwraps Google q= redirects", () => {
    expect(extract("https://www.google.com/url?q=https%3A%2F%2Fexample.com%2Fjobs%2F123"))
      .toEqual(["https://example.com/jobs/123"]);
  });

  it("unwraps Google url= redirects", () => {
    expect(extract("https://www.google.com/url?url=https%3A%2F%2Fexample.com%2Fjobs%2F123"))
      .toEqual(["https://example.com/jobs/123"]);
  });

  it("unwraps Bing redirects with url=", () => {
    expect(extract("https://www.bing.com/ck/a?url=https%3A%2F%2Fexample.com%2Fjobs%2F123"))
      .toEqual(["https://example.com/jobs/123"]);
  });
  ...
  it("unwraps DuckDuckGo uddg redirects", () => {
    expect(extract("https://duckduckgo.com/l/?uddg=https%3A%2F%2Fexample.com%2Fjobs%2F123"))
      .toEqual(["https://example.com/jobs/123"]);
  });
  ...
  it("deduplicates direct and redirect representations", () => {
    expect(extract("https://example.com/jobs/123 https://www.google.com/url?q=https%3A%2F%2Fexample.com%2Fjobs%2F123"))
      .toEqual(["https://example.com/jobs/123"]);
  });
  ...
  it("rejects obvious login/signup/account URLs", () => {
    expect(extract("https://example.com/login https://example.com/signup https://example.com/register https://example.com/account"))
      .toEqual([]);
  });
  ...
  it("preserves meaningful query parameters while removing obvious tracking", () => {
    expect(extract("https://example.com/opening?id=123&team=frontend&utm_source=google#details"))
      .toEqual(["https://example.com/opening?id=123&team=frontend"]);
  });

  it("supports mixed markdown, HTML, RSS and bare URLs", () => {
    const page = [
      "[A](https://example.com/jobs/1)",
      '<a href="https://example.com/jobs/2">B</a>',
