...
export interface AdzunaJobSourceOptions { appId: string; appKey: string; countryCode: string; pages?: number; resultsPerPage?: number; queries?: readonly string[]; locations?: readonly string[]; }
  ...
  async fetchJobs(signal?: AbortSignal): Promise<Job[]> {
    ...
    for (const query of queries) for (const location of locations) {
      for (let page = 1; page <= (this.options.pages ?? 1); page += 1) {
        if (signal?.aborted) throw new Error("Adzuna discovery aborted");
        ...
        const response = await fetch(`https://api.adzuna.com/v1/api/jobs/${encodeURIComponent(this.options.countryCode)}/search/${page}?${params}`, { signal, headers: { Accept: "application/json" } });
        ...
        for (const item of body.results ?? []) {
          ...
          jobs.push({ source: "adzuna", sourceJobId: string(item.id) ?? url, url, title, companyName: company ?? "Unknown company", companyDomain: null, location: loc ?? location ?? null, country: this.options.countryCode, workplaceType: null, employmentType: string(item.contract_type), description: string(item.description) ?? "", postedAt: date(item.created), updatedAt: null, contentHash: `${url}|${title}|${string(item.description) ?? ""}` });
        }
      }
    }
    return jobs;
