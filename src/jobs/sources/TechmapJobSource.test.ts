...
describe("TechmapJobSource", () => {
  ...
  it("supports portal-filtered normalized results", async () => {
    global.fetch = jest.fn().mockImplementation(async () => new Response(JSON.stringify({ result: [{ id: 1, title: "Frontend Engineer", url: "https://jobs.example/frontend", company: "Example", location: "Bengaluru, India", description: "React TypeScript", workPlace: "hybrid", dateCreated: "2026-09-01T00:00:00Z" }] }), { status: 200, headers: { "content-type": "application/json" } })) as typeof fetch;
  ...
  it("supports global corpus mode when no portal list is supplied", async () => {
    global.fetch = jest.fn().mockImplementation(async () => new Response(JSON.stringify({ result: [{ id: "g1", title: "React Developer", url: "https://jobs.example/react", company: { name: "Example" }, location: { city: "Bengaluru", country: "India" }, fullDescription: "React" }] }), { status: 200 })) as typeof fetch;
