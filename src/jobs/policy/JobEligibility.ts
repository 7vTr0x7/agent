...
function isTooOld(job: JobEligibilityInput, policy: JobSearchPolicy, now = new Date()): boolean {
...
export function evaluateJobEligibility(job: JobEligibilityInput, policy: JobSearchPolicy): JobEligibilityResult {
  ...
  if (isExcludedCompany(job.companyName, policy.excludedCompanies)) {
    return { decision: "REJECT", priority: null, reason: "Company is explicitly excluded from applications." };
  }
  if (isTooOld(job, policy)) {
    return { decision: "REJECT", priority: null, reason: `Job is older than the configured ${policy.maxAgeDays}-day freshness window or has a future posted date.` };
  }

  const risk = assessJobRisk({ title: job.title ?? "", companyName: job.companyName, description: job.description ?? "" });
  if (risk.level === "HIGH") {
    return { decision: "REJECT", priority: null, reason: `Job was rejected by the safety-risk policy: ${risk.reasons.join(" ")}` };
  }

  if (isBangalore(job, policy)) {
    return { decision: "ELIGIBLE", priority: 1, reason: risk.level === "MEDIUM" ? "Bangalore/Bengaluru is the highest-priority target location; the listing also carries a medium-risk warning." : "Bangalore/Bengaluru is the highest-priority target location." };
  }

  if (isRemote(job)) {
    if (!policy.allowRemote) return { decision: "REJECT", priority: null, reason: "Remote work is disabled by the job-search policy." };
    return { decision: "ELIGIBLE", priority: 3, reason: risk.level === "MEDIUM" ? "Remote role is allowed by the job-search policy; the listing also carries a medium-risk warning." : "Remote role is allowed by the job-search policy." };
  }

  if (isTargetCountry(job, policy)) {
    return { decision: "ELIGIBLE", priority: 2, reason: risk.level === "MEDIUM" ? "Role is located in the target country; the listing also carries a medium-risk warning." : "Role is located in the target country." };
  }
