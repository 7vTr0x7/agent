...
export class PostgresJobOpportunityRepository implements JobOpportunityRepository {
  ...
  async findById(id: string): Promise<JobOpportunity | null> {
    ...
    return result.rows[0] ? mapJobOpportunity(result.rows[0]) : null;
  }

  async findByCanonicalId(canonicalId: string): Promise<JobOpportunity | null> {
    const result = await this.database.query<JobOpportunityRow>(
      `${selectSql} WHERE canonical_id = $1`,
      [canonicalId]
    );
    return result.rows[0] ? mapJobOpportunity(result.rows[0]) : null;
  }
  ...
  async save(opportunity: JobOpportunity): Promise<JobOpportunity> {
    const result = await this.database.query<JobOpportunityRow>(
      `
        INSERT INTO job_opportunities (
          ...
          country, workplace_type, employment_type, description, posted_at,
          updated_at, last_seen_at, closed_at, status
        )
    ...
    if (!row) throw new Error("Failed to persist job opportunity");
    return mapJobOpportunity(row);
  }
}
