...
describe("TaskWorker", () => {
  ...
  it("logs lease loss without failing the worker loop", async () => {
    const task = { id: "task-2", taskType: "test", payload: {}, status: "RUNNING" as const, priority: 0, availableAt: new Date(), lockedAt: new Date(), leaseExpiresAt: new Date(), lockedBy: "worker-2", attempts: 1, maxAttempts: 3, dedupeKey: null, workerId: "worker-2" };
    const queue = { recoverStaleTasks: jest.fn().mockResolvedValue({ recovered: 0 }), claim: jest.fn().mockResolvedValue(task), heartbeat: jest.fn().mockResolvedValue(false), succeed: jest.fn().mockRejectedValue(new Error("Task is not owned by this worker")), fail: jest.fn().mockRejectedValue(new Error("Task is not owned by this worker")) } as unknown as TaskQueue;
    const logger: TaskWorkerLogger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
