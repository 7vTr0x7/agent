...
async function main():Promise<void>{
  ...
  try{
    ...
    let queued=0, skippedExcluded=0, skippedNoDomain=0, resolvedFromPersistedEvidence=0;
    console.log(`[requeue-recruiter-discovery] inspecting ${result.rows.length} jobs using persisted employer evidence only; public-web fallback skipped`);
    for(const job of result.rows){
      if(excluded(job.company_name)){skippedExcluded++;continue;}
      const companyDomain=resolveEmployerDomainFromJobData(job.company_domain,job.canonical_url,job.description);
      if(!companyDomain){skippedNoDomain++;continue;}
      resolvedFromPersistedEvidence++;
    ...
    console.log(JSON.stringify({status:"QUEUED",inspected:result.rows.length,queued,resolvedFromPersistedEvidence,skippedExcluded,skippedNoDomain,publicWebFallback:"SKIPPED",eligibility:"MATCH_DECISION_APPLY_OR_REVIEW",dedupeVersion:"v5-persisted-domain-only"},null,2));
