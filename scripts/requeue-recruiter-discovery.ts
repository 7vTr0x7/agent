...
async function main():Promise<void>{
  ...
  try{
    ...
    for(const job of result.rows){
      ...
      if(!companyDomain){
        companyDomain=await resolveEmployerDomainFromPublicSearch(job.company_name);
    ...
    console.log(JSON.stringify({status:"QUEUED",inspected:result.rows.length,queued,resolvedByPublicSearch,skippedExcluded,skippedNoDomain,eligibility:"MATCH_DECISION_APPLY_OR_REVIEW",dedupeVersion:"v3-public-employer-resolution"},null,2));
