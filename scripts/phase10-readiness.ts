...
async function main(): Promise<void> {
  ...
  if (databaseUrl) {
    ...
    try {
      ...
      add(checks, "Migrations", migration.rows[0]?.name === "040_public_contact_resources.sql" ? "READY" : "BLOCKED", migration.rows[0]?.name ? `Latest migration: ${migration.rows[0].name}.` : "No migration history found.");
