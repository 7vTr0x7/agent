...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(JSON.stringify({
      status:"ok", feature:"JOB_FIRST_APPLICATION_EMAIL", independentEmailBranch:true, sendEnabled:false, gmailEnabled:false, outboundEnabled:false,
...
async function resolveEmployerDomainFromPlatformCompanyPage(jobUrl: string, companyName: string): Promise<{domain:string;sourceUrl:string}|null> {
  let parsed: URL;
  try { parsed = new URL(jobUrl); } catch { return null; }
  ...
  try { validatedPageUrl = await validatePublicHttpUrl(companyPageUrl); } catch { return null; }
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 15_000);
  try {
    let html: string | null = null;
    const response = await fetch(validatedPageUrl, { signal: controller.signal, headers: { accept:"text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", "user-agent":"Mozilla/5.0 (compatible; JobAgent/0.1; +https://github.com/7vTr0x7/agent)" } });
    if (response.ok) html = await response.text();
    else if (response.status === 403 || response.status === 429 || response.status >= 500) {
      const readerResponse = await fetch(`https://r.jina.ai/${validatedPageUrl}`, { signal: controller.signal, headers: { accept:"text/plain,text/html;q=0.9,*/*;q=0.8", "user-agent":"Mozilla/5.0 (compatible; JobAgent/0.1; +https://github.com/7vTr0x7/agent)" } });
      if (readerResponse.ok) html = await readerResponse.text();
    }
    if (!html) return null;
