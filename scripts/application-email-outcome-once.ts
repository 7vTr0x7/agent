...
type ApplicationCandidate = {
  job_opportunity_id: string; company_name: string; title: string; canonical_url: string;
  company_domain: string | null; description: string; location: string | null; country: string | null;
  workplace_type: "onsite" | "remote" | "hybrid" | null; employment_type: string | null;
  posted_at: Date | null; updated_at: Date | null;
};
...
async function enrichEmployerDomain(database: Database, job: ApplicationCandidate, events: Array<Record<string, unknown>>): Promise<ApplicationCandidate> {
  ...
  try {
    ...
    events.push({ event: "application-employer-domain-resolution-error", company: job.company_name, jobOpportunityId: job.job_opportunity_id, reason: error instanceof Error ? error.message : String(error), stage: "trusted-job-source" });
  }

  try {
    const enricher = new JobDetailEnricher({ timeoutMs: 15_000, concurrency: 1, maxRedirects: 3 });
    const result = await enricher.enrich({
      source: "application-preflight", sourceJobId: job.job_opportunity_id, url: job.canonical_url,
      title: job.title, companyName: job.company_name, location: job.location, country: job.country,
      workplaceType: job.workplace_type, employmentType: job.employment_type, description: job.description,
      postedAt: job.posted_at, updatedAt: job.updated_at, contentHash: job.job_opportunity_id
    }, undefined, true);
    ...
    events.push({ event: "application-employer-domain-resolution-error", company: job.company_name, jobOpportunityId: job.job_opportunity_id, reason: error instanceof Error ? error.message : String(error), stage: "job-detail" });
  }
  return job;
}
  ...
  try {
    ...
    const target = await database.query<ApplicationCandidate>(
      `SELECT jo.id AS job_opportunity_id, jo.company_name, jo.title, jo.canonical_url, jo.company_domain,
              jo.description, jo.location, jo.country, jo.workplace_type, jo.employment_type, jo.posted_at, jo.updated_at
       FROM job_opportunities jo
       JOIN match_decisions md ON md.job_opportunity_id=jo.id AND md.candidate_profile_id=$1
       WHERE md.decision='APPLY' AND jo.status='ACTIVE' AND jo.canonical_url IS NOT NULL AND jo.canonical_url <> ''
       ORDER BY (jo.company_domain IS NOT NULL AND jo.company_domain <> '') DESC, md.match_score DESC, jo.posted_at DESC NULLS LAST
       LIMIT 12`, [profile.id]
    );
    ...
    for (const candidate of target.rows) {
      ...
      if (enriched.company_domain) { job = enriched; break; }
    ...
    if (!job) events.push({ event: "application-email-discovery-blocked", company: selected.company_name, reason: "validated employer-domain evidence unavailable for the APPLY candidates; application may continue safely without email discovery" });

    const queue = new TaskQueue(database);
    const taskId = await queue.enqueue({ taskType: APPLY_JOB_TASK, payload: { jobOpportunityId: selected.job_opportunity_id, candidateProfileId: profile.id }, priority: 100, dedupeKey: `fast-application-email:${selected.job_opportunity_id}:${profile.id}` });
    ...
    const handler = new ApplicationTaskHandler(applications, submissions, ConfiguredCandidateProfileResolver.fromEnvironment(), excludedCompanies, emailDispatcher, undefined, undefined, undefined, undefined, undefined, emailDiscovery);
    ...
    try { await handler.handle(claimed); } catch (error) { handlerError = error instanceof Error ? error.message : String(error); }
    const applicationRow = await database.query<{ status: string; id: string }>(`SELECT id,status FROM applications WHERE job_opportunity_id=$1 AND candidate_profile_id=$2 ORDER BY created_at DESC LIMIT 1`, [selected.job_opportunity_id, profile.id]);
    if (handlerError) await queue.fail(claimed.id, workerId, handlerError); else await queue.succeed(claimed.id, workerId);

    console.log(JSON.stringify({
      status: "ok", feature: "JOB_FIRST_APPLICATION_EMAIL", independentEmailBranch: true,
      sendEnabled: false, gmailEnabled: false, outboundEnabled: false,
      target: { jobOpportunityId: selected.job_opportunity_id, company: selected.company_name, title: selected.title, jobUrl: selected.canonical_url, companyDomain: selected.company_domain },
      ...
      emailDiscovery: { started: emailDiscoveryStarted, finished: emailDiscoveryFinished, blocked: !selected.company_domain, contacts: discoveredEmails.length, contactsWithEmail: discoveredEmails.filter(c => Boolean(c.email)).length, results: discoveredEmails.slice(0, 10) },
      preparedEmail: preparedEmails[0] ?? null, events
    }, null, 2));
