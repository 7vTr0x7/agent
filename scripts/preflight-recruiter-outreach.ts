...
function main(): void {
  ...
  if (!config.recruiterOutreach.dryRun && config.recruiterOutreach.requireVerifiedEmail !== true) fail("real recruiter outreach requires RECRUITER_REQUIRE_VERIFIED_EMAIL=true.");
  if (!config.recruiterOutreach.dryRun && config.recruiterOutreach.minConfidence < 80) fail("real recruiter outreach requires RECRUITER_MIN_CONFIDENCE>=80.");
  if (!config.recruiterOutreach.dryRun && config.recruiterOutreach.maxContactsPerApplication > 3) fail("real recruiter outreach allows at most 3 contacts per application.");
  if (config.recruiterOutreach.maxMessagesPerDay > config.gmail.dailySendLimit) fail(`RECRUITER_MAX_MESSAGES_PER_DAY=${config.recruiterOutreach.maxMessagesPerDay} exceeds the configured Gmail ${config.gmail.accountTier} daily limit of ${config.gmail.dailySendLimit}.`);
  if (config.recruiterOutreach.maxMessagesPerHour > config.recruiterOutreach.maxMessagesPerDay) fail("RECRUITER_MAX_MESSAGES_PER_HOUR cannot exceed the daily recruiter send limit.");
  if (!config.recruiterOutreach.dryRun && config.recruiterOutreach.activation === "live" && config.recruiterOutreach.maxMessagesPerHour < Math.ceil(config.recruiterOutreach.maxMessagesPerDay / 24)) checks.push("hourly recruiter limit is below daily/24 pacing; configured live throughput will be lower than the Gmail daily ceiling.");
